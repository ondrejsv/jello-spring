...
//try{var ctx=getTagName(tid);}catch(e){ctx="Jello Objects";}
    if (istag == false) {
        buildGrid(ctx, null, 0, filtering, oItems, istag);
    }
    if (istag == null) {
        istag = true;
    }
...
//take care and check context string
    var pCtx = ctx;
    if (fun == 1 || fun == 2) {
        window.status = txtFetching;
    } else {
        window.status = txtCounting;
    }
//set DASL filter
    if (dasl == null) {
...
//no filters
    ...
    if (fun == 0) {
        ...
        status = txtReady;
        if (fun == 1) {
            buildGrid(ctx, its, counter, filtering, oItems, istag);
            return;
        }
        if (fun == 2) {
...
//open action edit form
    ...
    getTagsArray();
    try {
        var t = Ext.getCmp("theactionform");
        t.destroy();
    } catch (e) {
    }
    thisViewType = "actionform";
    try {
        var itcount = actionItems.length;
    } catch (e) {
        itcount = 0;
    };
    if (itcount > 0) {
        if (actionItems[0].get("type") == "t") {
            editTag(actionItems);
            return;
        }
    }
    ...
    if (jello.dateFormat == 0 || jello.dateFormat == "0") {
        dccformat = "j" + dts + "n" + dts + "y";
        dccalt = "d/m|j/n|d-m|j-n|d/m/y|d/m/y";
    }
    if (jello.dateFormat == 2 || jello.dateFormat == "2") {
        dccformat = "Y" + dts + "m" + dts + "d";
        dccalt = "d/m|n/j|n-j|y/m/d|y/m/d/Y-m-d";
    }
...
//handle new item
        ...
        if (assignLast != false) {
            ftagList = lastContext + ";";
            tgs = tagList(null, null, ftagList);
        } else {
        }
        if (notEmpty(assignTag)) {
            ftagList = assignTag;
            tgs = tagList(null, null, ftagList);
        }
...
//handle one item
        try {
            var it = NSpace.GetItemFromID(actionItems[0].get("entryID"));
        } catch (e) {
            return;
        }
        ...
        if (it.Class == 43)
            ...
            try {
                var tt = new Date(it.itemProperties.item(dueProperty).Value);
            } catch (e) {
            }
        fdue = tt;
        if (fdue.getYear() > 4000) {
            fdue = null;
        }
...
//handle multiple items
    ...
    if (jello.autoUpdateTaskNotes == false) {
        notesStatus = txtOLNoBodyReplace;
    }
...
//show the form
    ...
    setTimeout(function() {
        try {
            Ext.getCmp("subj").focus(true, 100);
        } catch (e) {
        }
    }, 1);
...
//action form save button handler
    ...
    if (returnID != true) {
        ...
        if (notEmpty(Ext.getCmp("isSaved").getValue())) {
            cl = 1;
        }

        try {
            if (Ext.getCmp("olid").getValue() == "000") {
                cl = 0;
                var ids = new Array();
            }
        } catch (e) {
    ...
    if (cl == 0) {
        if (Ext.getCmp('subj').getValue() == txtNewAction) {
            Ext.getCmp('subj').focus(true);
            alert(txtErrNoActionName);
            return false;
        }
        ...
        //called from grid
        {
            ...
            var nrr = new Array();
            nrr.push(newRec);
            g.getSelectionModel().selectRecords(nrr, true);
        }
...
//update action to outlook task
    ...
    if (oldNotes != Ext.getCmp('notes').getValue()) { //if notes were changed update
...
//if (btn.id!="adel"){return;}
    ...
    if (btn.id == "adel" || btn.id == "acmdel") {
        if (choice = confirm(txtMsgDelItem) == false) {
            status = txtReady;
            return;
        }
    }
...
//for edit selected open form for bulk edit
    ...
    for (var x = 0; x < cl; x++) {
        ...
        if (btn.id == "adel" || btn.id == "acmdel") {
            ...
            if (itype == "t") {
                deleteTag(id, null, Ext.getCmp("grid"));
                rmv = true;
            }
        }
        //toggle NA
        if ((btn.id == "anext" || btn.id == "acmnext") && itype == "a") {
            aFailed = doAction(id);
        }
        //toggle Done
        if ((btn.id == "adone" || btn.id == "acmdone") && itype == "a") {
        ...
        //Convert to project
        if (btn.id == "aproject") {
            aFailed = convertToProject(id);
        }
        //copy action to another tag
        if (btn.id == "acopyaction") {
            editems.push(id);
        }
        //toggle Priority
        if (btn.id == "apri" && itype == "a") {
            aFailed = setActionPriority(id);
        }
        //add contacts
        if (btn.id == "adelg" && itype == "a") {
            editems.push(cit[x]);
        }
        //set priority by number key
        if (btn == "numaction" && itype == "a") {
            aFailed = setActionPriority(id, vl);
        }
        //toggle Start/stop
        if (btn.id == "astart" && itype == "a") {
            aFailed = startStopAction(id);
        }
        //toggle Privacy
        if (btn.id == "alock" && itype == "a") {
            aFailed = lockAction(id);
        }
        //open in outlook
        if (btn.id == "aoutlook" || btn.id == "acmoutlook") {
            olItem(id);
        }
        //create appointment
        if (btn.id == "aappoint") {
        ...
        if (btn == "addtag") {
            ...
            if (notEmpty(tg)) {
                if (addTagToAL(tg, it, cit[x], false) == false) {
                    status = txtReady;
                    return;
                }
            }
    ...
    if (btn.id == "adelg") {
        setActionContact(id, editems);
    }
    if (btn.id == "acopyaction") {
        copyActionsToTag(id, editems);
    }
    if (btn.id == "aproject") {
        refreshView();
    }
...
//if action was not delete reopen form updated
        if ((btn.id != "adel" || btn.id == "acmdel") && (btn.id != "adone" || btn.id == "acmdone")) {
            editAction(cit);
        } else {
...
//grid selection
    ...
    if (gridid == null && typeof(olview) == "undefined") {
        olview = "olnative";
    }
        ...
        if (olview.substr(0, 4) == "olv:" || olview == "olnative") {
            ...
            try {
                if (olview == null) {
                    ...
                    try {
                        var olv = olm.Selection;
                    } catch (e) {
                        alert("IE 7 now blocks handling of Outlook view control contents. You can manage contents only by the built in keyboard shortcuts or context menus.");
                    }
                }

                for (var x = 1; x <= (olv.Count); x++) {
                    if (!notEmpty(olv.Item(x))) {
                        olv.Item(x).Subject = txtNoSubject;
                        olv.Item(x).Save();
                    }
                    sr.push(olv.Item(x));
                }
                ...
                Ext.info.msg(e + " " + txtAlOLCtlError, txtInvalid);
                return false;
            }
        }
    }
...
//quick action addition from toolbar
    ...
    if (newAction == null || newAction == "" || newAction == " ") {
        qaf.focus();
        return;
    }

    var g = getActiveGrid();
    if (trimLow(newAction.substr(0, 4)) == "ref:") {
        addJournalInTag(newAction.substr(4, 200));
        qaf.reset();
        qaf.focus();
        return;
    }
    if (trimLow(newAction.substr(0, 1)) == "#") {
        addJournalInTag(newAction.substr(1, 200));
        qaf.reset();
        qaf.focus();
        return;
    }
...
function createActionOL(newAction) { //j5
...
//var e=i.tags;
    ...
    e = trimLow(e);
    try {
        var ss = e.search(trimLow(getActionTagName()));
    } catch (b) {
        var ss = -1;
    }
    if (ss != -1) {
...
//set-unset task for next action (star)
    ...
    var cat2 = "";
    try {
        var ss = trimLow(cat).search(trimLow(getActionTagName()));
    } catch (e) {
        var ss = -1;
    }
    if (a == null && ss > -1) {
    ...
    if (stat == 0) {
        ...
        if (jello.nextToggleHighPri == 1 || jello.nextToggleHighPri == "1") {
            setActionPriority(id, 0);
        }
    } else {
        /*set*/
        cat2 = cat + ";" + getActionTagName();
        cat2 = cat2.replace(new RegExp(",", "g"), ";");
        if (jello.nextToggleHighPri == 1 || jello.nextToggleHighPri == "1") {
            setActionPriority(id, 1);
        }
    }
...
function actionDone(id) { //j5
//Toggle action as done

    try {
        var pn = NSpace.GetItemfromID(id);
    } catch (e) {
        alert(txtInvalid);
        return true;
    }
...
function actionObject(eID, other) { //j5
//add action items (Tasks) to itms array

// if a Note
    if (eID.Class == 44) { //notes need different handling
        this.iclass = 44;
        this.subject = eID.Subject;
        this.entryID = eID.EntryID;
        this.body = eID.body;
        this.created = new Date(eID.LastModificationTime);
...
//journal entries
        ...
        this.iclass = 42;
        this.subject = "(" + DisplayDate(jdd) + ") " + eID.Subject;
        this.entryID = eID.EntryID;
        this.created = new Date(eID.LastModificationTime);
        this.icon = this.iclass + ";0;0";
        this.type = "j";
        try {
            this.tags = eID.itemProperties.Item(catProperty).Value;
        } catch (e) {
        }
        ...
        try {
            this.attachmentCount = eID.Attachments.Count;
        } catch (e) {
        }
        return;
    }
    ...
    try {
        this.entryID = eID.EntryID;
    } catch (e) { //case of imap or other non downloaded items
        // also handles case of an item handled by an addon
        // that isn't a valid item
        try {
            this.subject = eID.Subject;
        } catch (e) {
            this.subject = txtBadItem;
        };
    ...
    if (eID.Class == 43) {
        ...
        try {
            this.markfordownload = eID.RemoteStatus;
        } catch (e) {
        }
        if (eID.FlagStatus == 2) {
            this.flag = 1;
...
//this in case of security prompts of outlook
        ...
        if (ats.Count > 0) {
            ...
            for (var i = 1; i <= ats.Count; i++) {
                var z = ats.Item(i);
                try {
                    this.attachmentList += colon + "&nbsp;<a onclick=olItem('" + eID.EntryID + "'); class=jelloLink style='color:blue;text-decoration:underline;'>" + z.FileName + "</a>";
                } catch (e) {
                }
                colon = ";";
            }
...
// natural sort order for contact is fullname, but createtime should be used
    ...
    this.created = t;
    if (eID.Class == 48) {
        var tdc = new Date(eID.DateCompleted);
    } else {
        var tdc = null;
    }
    this.Complete = tdc;
    ...
    try {
        var t = new Date(eID.itemProperties.item(dueProperty).Value);
    } catch (e) {
    }
    var dd = DisplayDate(t);
    if (dd == null || dd == "") {
    ...
    //non task
    {
        ...
        try {
            if (eID.Links.Count > 0) {
                ...
                if (notEmpty(this.contacts)) {
                    this.contacts = this.contacts.substr(0, this.contacts.length - 1);
                }
    ...
    try {
        this.attachment = eID.UserProperties.Item("OLID").value;
    } catch (e) {
    }
    try {
        this.attachmentCount = eID.Attachments.Count;
...
//set contacts to action
    ...
    if (recs.length == 1) { //if its one record to be updated, update contact list to show existing contacts
...
//assign icon to an outlook item
    ...
    try {
        ...
        if (c == 43) {
            ic = icMail;
            var icem = "<img src=img\\" + ic + ">";
            //follow up flag icons
            var fl = r.get("flag");
            if (fl == 1) {
                icem = "<span style='background-image:url(" + imgPath + ic + ");background-repeat:no-repeat;filter:alpha(OPACITY=20)'><img title='Flagged' src=img\\flag.gif></span>";
            }
            if (fl == 10) {
                icem = "<span style='background-image:url(" + imgPath + ic + ");background-repeat:no-repeat;filter:alpha(OPACITY=20)'><img title='Flagged' src=img\\check.gif></span>";
            }
            return icem;
        }
        ...
        if (c == null) {
            ic = icApp;
            return "<img src=img\\" + ic + ">";
        }
        if (c == 48) {
            if (cst == 0 && jello.showNotStartedAlways == false) {
                ic = icTaskNS;
            }
            if (cse == 2) {
                ic = icTaskP;
        ...
        if (c == 999) { //encrypted or unable to read items
            ic = "icon_key.gif"
        }
...
//attached items (for multi action tasks!) Try to have this in action object
    ...
    if (typeof(t) == "undefined") { //tag icons
        var i = r.get("iclass");
    ...
    try {
        if (isOLReccuring(r.get("entryID"))) {
            ic = "taskrec.gif";
        }
    } catch (e) {
    }
...
//return context or project icon
    try {
        var ix = tagStore.find("tag", ctx);
        var r = tagStore.getAt(ix);
        var icon = r.get("icon");
        if (notEmpty(icon)) {
            return "<img src='img\\" + icon + "' style=float:left;>";
        }
        ...
        case 7:
            ...
            break;
        case 8:
            icc = "list_waiting.gif";
            break;
        case 9:
            icc = "list_someday.gif";
            break;
        }
        return "<img src=img\\" + icc + " style=float:left;>";
    } catch (e) {
        return "<img src=img\\icon_extension.gif style=float:left;>";
    }
}
...
//get custom notes field value. Create if not exists
    ...
    try {
        ...
        it.UserProperties.Add("jnotes", 1, true);
    }
...
function getItemBody(it) {
    if (it.Class == 43) {
        ...
        if (jello.autoUpdateSecuredFields == false || jello.autoUpdateSecuredFields == 0 || jello.autoUpdateSecuredFields == "0") {
            ...
            //otherwise this
            if (jello.mailPreviewHTML == true) {
                oln = it.HTMLBody;
            } else {
                oln = cleanBody(it.Body);
            }
        }
        return oln;
    ...
    if (!isWaiting) {
        ...
        if (oItems != null) {
            for (var x = 0; x < oItems.length; x++) {
                var it = null;
                try {
                    it = NSpace.GetItemFromID(oItems[x].get("entryID"));
                } catch (e) {
                }

                var ar = null;
                if (!notEmpty(it)) {
                    ar = new actionObject(oItems[x], true);
                } else {
                    ar = new actionObject(it);
                }
                var newRec = new actionRecord(ar);
...
//insert reference items
    ...
    if (filtering != "0") {
        updateCounter("(" + txtFilterOn + ") " + cc);
    }
    var fltr = ctx;
    var dts = jello.dateSeperator;
    var ALinline = false;
    if (jello.ALinlineduedate == 1 || jello.ALinlineduedate == "1") {
        ALinline = true;
    }
    var fmt = "n" + dts + "j" + dts + "y";
//context
    if (filtering == "99") {
        fltr = lastContext + " " + txtFilterOn + " " + ctx;
    }
    var hasParent = null;
    ...
    setTimeout(function() {
        try {
            if (jello.selectFirstItem == 1 || jello.selectFirstItem == "1") {
...
//insert focuses the new action toolbar box
    var lmap2 = new Ext.KeyMap(gridName, {
        ...
        fn: function() {
            Ext.getCmp("inlinenewaction").focus();
            try {
                var aw = Ext.getCmp("subj");
                aw.focus();
            } catch (e) {
            }
        },
...
function actionContextMenu(rec, id, inlist) {
    try {
        Ext.getCmp("acmrclickmenu").destroy();
    } catch (e) {
    }
...
//get item's importance and return color, and attachment icon next to it
    ...
    if (acou > 0) {
        aticon = "<a class=jellolink title='" + acou + " " + txtAttachments + "' onclick='openInboxItem(\"" + id + "\");'><img src=" + imgPath + "attach.gif></a>";
    }
    var ic = "";
    if (v == 1) {
        ic = "red";
...
//render the subject line in grid
    ...
    var delreply = "";
    if (dstate == 1) {
        delreply = txtDelgStatSent;
    }
    if (dstate == 2) {
        delreply = txtDelgStatAcc;
    }
    if (dstate == 3) {
        delreply = txtDelgStatDec;
    }
    if (dstate > 0) {
        subj = "<img src=img//inboxmail.gif><font size=1 color=green>(" + delreply + ")</font> " + subj;
    }

    if (cl == 42) {
        rt = subj + "</span>&nbsp;" + tList[0];
        return rt;
    }
    var conicon = "";
    var con2 = con.replace(new RegExp("'", "g"), " ");
    if (notEmpty(con) && con != "(" + txtContactNo + ")") {
        conicon = "<a class=jellolink title='" + con2 + "'><img src=" + imgPath + "user.gif></a>";
    }
...
//open row record for editing
    try {
        var sr = g.getSelectionModel().getSelections();
        if (sr.length == 1) { //if selection is one tag navigate instaed of showing props
...
function refreshView() {
    ...
    if (g.getId() == "grid") {
        if (lastContext == "##Inbox") {
            showNoMailInbox(getInboxTaskDASL());
            return;
        }
...
//remove an attachment
    try {
        ...
        Ext.info.msg(txtError, txtMsgNoRmvAtt);
    }
}
...
//print actionlist grid
    ...
    var toPrint = function(rec) {
        try {
            var ddd = DisplayDate(rec.get("due"));
        } catch (e) {
            var ddd = "";
        }
        try {
            var ttt = rec.get("body");
        } catch (e) {
            var ttt = "";
        }
        try {
            var icls = rec.get("iclass");
        } catch (e) {
            var icls = "";
        }
...
//lastOpenTagID
    try {
        it.Subject = lastContext + " " + txtCbActions;
    } catch (e) {
        it.Subject = txtCbActions;
    }
    it.HTMLBody = ret;
    if (direct == true) {
        it.PrintOut();
        Ext.info.msg(txtPrinting, txtPrintOK);
    } else {
        it.Display();
    }
...
//export all/selected items to a folder. Further Selections
    ...
    if (ff.DefaultItemType != 3) {
        alert(txtPromptTaskFolder);
        return;
    }
    ...
    for (var x = 0; x < rex.length; x++) {
        ...
        try {
            ...
            if (fit.Count > 0) {
                ...
                var it2 = fit(1);
                var db = new Date(it2.LastModificationTime);
...
function cleanBody(bdy) { // take a plain text body and convert to HTML for preview display
    var s = bdy;
    //;strip the img tags
    var re = /<img [^>]+>/ig;
    s = s.replace(re, "");
...
//olit is tickler specific (a recurrence)
    ...
    if (wasSel)
        setTimeout(function() {
            ...
            try {
                ag.getSelectionModel().selectRow(idx);
                ag.getView().focusRow(idx);
                ag.focus();
            } catch (e) {
            }
        }, 6);
...
function getFirstAttached(att) {
    at = att.split(";");
    try {
        var rt = NSpace.GetItemFromID(at[0]);
        return rt;
    } catch (e) {
        return null;
    }
...
//print list to not as plain text
    ...
    if (g.getId() == "grid") {
        ...
        var toTxtPrint = function(rec) {
            ...
            try {
                var ttt = rec.get("tags");
            } catch (e) {
                var ttt = "";
            }
            try {
                var tbt = rec.get("body");
            } catch (e) {
                var tbt = "";
            }
            if (cl > 99) {
...
function saveGridState(grd) { //save widths and showed columns of a grid
    try {
        var g = Ext.getCmp(grd);
        var sortOrder = null;
        var st = g.getStore().getSortState();
        var sfld = st.field;
        var sord = st.direction;
...
function restoreGridState(grd) { //restore saved column widths and hidden status of a grid
    var g = Ext.getCmp(grd);
    var store = g.getStore();
    var idx = columnStore.find("grid", new RegExp("^" + grd + "$"));
    if (idx == -1) {
    ...
    for (var i = 0; i < newConfig.length; i++) {
        ...
        try {
            cm.setHidden(i, newConfig[i].hidden);
        } catch (e) {
        } //Hidden
        cm.setColumnWidth(i, newConfig[i].width); // Widths
        try {
            if (newConfig[i].sorted != null) {
                doSortFld = newConfig[i].dataIndex;
                doSortOrd = newConfig[i].sorted;
            }
        } catch (e) {
        }
    }
    ...
    try {
        store.sort(doSortFld, doSortOrd);
    } catch (e) {
    }
}
...
//handle toolbar buttons of action form
    ...
    if (btn.id.substr(0, 4) == "frm-") {
        btn.id = btn.id.substr(4, 10);
    }
    ...
    if (btn.id == "adone") {
        ...
        aFailed = actionDone(id);
        var comp = it.Complete;
        var xt = "";
        if (comp) {
            Ext.getCmp("theactionform").destroy();
        } else {
            alert(txtAction + " " + txtUncompleted);
        }
    }
    //toggle Priority
    if (btn.id == "apri") {
    ...
    if (btn.id == "astart") {
        aFailed = startStopAction(id);
        if (it.Status == 0) {
            alert(txtAction + ":" + txtNotStarted);
        } else {
            alert(txtAction + ":" + txtInProgress);
        }
    }
    //toggle Privacy
    if (btn.id == "alock") {
        aFailed = lockAction(id);
        if (it.Sensitivity == 2) {
            alert(txtAction + ":" + txtMarkContextPrivate);
        } else {
            alert(txtAction + ":Non Private");
        }
    }
    //open in outlook
    if (btn.id == "aoutlook") {
...
function checkForExistingGridToUpdate(grd, it) {
    ...
    if (relevantGrid) {
        ...
        var nrr = new Array();
        nrr.push(newRec);
        grd.getSelectionModel().selectRecords(nrr, true);
        try {
            updateAFooterCounter(store);
        } catch (e) {
        }
            ...
            new Ext.form.DateField({
                ...
                listeners:
                {
                    change: function(f, nv) {
                        var rt = wpWeekInfo(nv);
                        Ext.getCmp("weekinfo").getEl().update(rt);
                    }
                }
            }),
            new Ext.form.Label({
    ...
    var toCalendar = function(rec) {
        ...
        if (rec.get("allday") == false && cl != 43 && cl != 48) {
            try {
                ...
                tmm = sd.format("H:i") + "&nbsp;";
                if (jello.timeFormat == 1 || jello.timeFormat == "1") {
                    tmm = sd.format("g:i") + "&nbsp;";
                }
            } catch (e) {
            }
...
//list
    ...
    var toPrint = function(rec) {
        ...
        if (bodyLimit != 0) {
            ttt = ttt.replace(/\n/g, ' ');
            ttt = ttt.substring(0, bodyLimit);
        }
        if (col == 0) {
            ret += "<tr>";
        }
        var ccc = " (" + rec.get("tags").replace(lastContext, "") + ")";
        if (ccc == " ()") {
            ccc = "";
        }
...
//lastOpenTagID
    ...
    enddatedisp = enddatedisp.replace(/\<\/span\>/, "");
    try {
        it.Subject = firstContext + " " + txtCbActions + ":" + txtWeek + "  " + stdatedisp + " - " + enddatedisp;
    } catch (e) {
        it.Subject = txtCbActions;
    }
    it.HTMLBody = ret;
...
//destroy form
    ...
    if (g.getId() == "mgrid")
        ...
        setTimeout(
            function() {
                showContext(firstContext, 1);
            }, 6);
}


function popTaskBody(id, rec) {
    if (jello.expBodyView == "1" || jello.expBodyView == 1) {
        popTaskBodyExperimental(id, rec);
        return;
    }
        ...
        buttons: [
            {
                ...
                listeners: {
                    click: function(b, e) {
                        ...
                        if (g.viewval == "text") {
                            x.body.dom.innerHTML = it.HTMLBody;
                            g.viewval = "html";

                        } else {
                            x.body.dom.innerHTML = cleanBody(it.Body);
                            g.viewval = "text";
                        }
    ...
    var tbar = [
        ...
        },
        {
            ...
            listeners: {
                click: function(b, e) {
                    try {
                        selFolder(txtCopy, popCopyMessage, null, false);
                    } catch (e) {
                    }
                }
            }
...
// if no selected item or requested item is start of first page of messages
    if (g.hasSelection() == false || (idx <= 0 && lPage <= 1)) {
        g.selectFirstRow();
    }
...
function actionDateEdited(obj) {
    ...
    try {
        var it = NSpace.GetItemFromID(obj.record.get("entryID"));
    } catch (e) {
        return;
    }
...
//show the form
    ...
    setTimeout(function() {
        try {
            Ext.getCmp("copytags").focus(true, 100);
        } catch (e) {
        }
    }, 1);
...
function copyTheActions(b, recs) {
    ...
    if (nTag == null) {
        ...
        if (ntt == false) {
            alert("This tag cannot be added. Aborting!");
            Ext.getCmp("theactioncopyform").destroy();
            return;
        } else {
            alert("Tag " + toTag + " created");
        }
    }
    var noRec = false;
    var counter = 0;
    for (var x = 0; x < recs.length; x++) {
        ...
        try {
            ...
            alert("Could not copy the Item " + it.Subject);
        }
    }
