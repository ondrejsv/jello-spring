...
// grow:true,
        ...
        listeners: {
            keydown: function(f, e) {
                if (e.getKey() == e.ENTER) {
                    if (!jello.collectAdvancedMode)validateLine();
                }
            }
        }
    });
...
function addCollectToolbar() {
    ...
    var dateMenu = new Ext.menu.DateMenu({
        handler: function(dp, date) {
            ...
            if (jello.collectAdvancedMode)
                tt += " ";
            else if (tt.substr((tt.length - 1), 1) != ",") {
                tt += ",";
            }
            var dt = date.format('m/j/Y');
            if (jello.dateFormat == 0) {
                dt = date.format('j/m/Y');
            }
            if (jello.dateFormat == 2) {
...
//outlook folder items import/collect form
    ...
    try {
        var folderName = NSpace.GetFolderFromID(pfid).FolderPath;
    } catch (e) {
        var folderName = "";
    }
    if (notEmpty(parc) == false) {
        parc = false;
    }
    if (notEmpty(prem) == false) {
        prem = false;
    }
    if (notEmpty(pcre) == false) {
        pcre = true;
    }
    if (notEmpty(pdim) == false) {
        pdim = false;
    }
    if (notEmpty(pdon) == false) {
        pdon = false;
    }
    if (notEmpty(pdup) == false) {
        pdup = false;
    }
...
//popup outlook folder selection to collect from
    ...
    if (t != null) {
        if (t.EntryID == jello.inboxFolder || t.EntryID == jello.archiveFolder || t.EntryID == jello.actionFolder) {
...
//update counter of items to be imported
    if (t == null) {
        ...
        if (notEmpty(id) == false) {
            return;
        }
        var t = NSpace.GetFolderFromID(id);
    }
    var simple = Ext.getCmp("olcolform");
    Ext.getCmp("marc").hide();
    Ext.getCmp("marc").setValue(false);
    Ext.getCmp("mrem").hide();;
    Ext.getCmp("mrem").setValue(false);
    Ext.getCmp("mdon").hide();

    if (t.DefaultItemType == 0) { //mail folder selected
        Ext.getCmp("marc").show();
    }

    if (t.DefaultItemType == 3) { //task folder selected
...
//test filter
    ...
    if (notEmpty(iDasl)) {
        ...
        if (tc.Count > 0) {
            tcc = "(" + txtFilterTo + " " + tc.Count + ")";
        } else {
            tcc = "(" + txtColAlFltNoItms + ")";
        }
    }
...
//dup=replace duplicates
    ...
    if (notEmpty(fid) == false) {
        alert(txtMsgNoFolder);
        return;
    }
    ...
    if (arc == true) {
        ...
        if (notEmpty(arcfolder) == false) {
            alert(txtNoArchiveFolder);
            return;
        } else {
...
//GET OUTLOOK FOLDER
    ...
    var icount = 0;
    try {
        var addTags = Ext.getCmp("cas").getValue();
    } catch (e) {
        var addTags = "";
    }
...
//apply filters
    if (notEmpty(dasl)) {
        var tt = t.Items.Restrict("@SQL=" + dasl);
    } else {
        var tt = t.Items;
    }
    ...
    if (tt.Count > 0) {
        for (var x = tt.Count; x >= 1; x--) {
            ...
            if (ttype == 3) {
                ...
                //copy original task
                {
                    var newoione = oi.Copy();
                    newoi = newoione.Move(taskfol);
                }
                ...
                if (cre == true) {
                    addTags += getUniqueImportedCategories(newcats, addTags);
                }
                if (dup == true) {
                    checkDuplicates(newoi, origDate, origParent);
                }
                ...
                //check for categories if not exists as tags
                if (cre == true && dim == false) {
                    addTags += getUniqueImportedCategories(cats, addTags);
                }
                ...
                if (dup == true) {
                    checkDuplicates(newA, origDate, origParent);
                }
            }
    ...
    try {
        Ext.getCmp("olcollectform").destroy();
    } catch (e) {
    }
    if (icount > 0) {
        Ext.getCmp("cinb").setText("<font size=2 color=blue>" + txtInbox + "&nbsp;(" + countInboxItems() + ")");
    }
}
...
//set custom attachment field value.
    ...
    try {
        ...
        it.UserProperties.Add("OLID", 1, true);
    }
    if (append && notEmpty(jn)) {
        val = jn + ";" + val;
    }
...
//tagStore.filter("tag",tg);
    ...
    if (tagStore.getCount() > 0) {
        return tagStore.getAt(0).get("tag");
    }
...
// 	}else{
    ...
    if (datenoise.toLowerCase() == txtNext.toLowerCase()) {
        ...
        if (slower == txtWeek.toLowerCase()) {
            dxn = dx;
            dxn.setDate(dxn.getDate() + 7);
        } else if (slower == txtMonth.toLowerCase()) {
            dxn = dx;
            dxn.setDate(dxn.getMonth() + 1);
        } else if (slower == txtYear.toLowerCase()) {
            dxn = dx;
            dxn.setDate(dxn.getFullYear() + 1);
        }
        if (dxn != null)
...
//on confirm mode display stats and return confirmation dialog choice
    ...
    var icount = 0;
    if (notEmpty(ct) == false) {
        alert(txtMsgImportNothing);
        return;
    }
//split lines
    var t = ct.split("\n");
    ...
    for (var x = 0; x < lline; x++) {
        ...
        } else { //if blank before first comma mark invalid
            if (l.length > 1) {
                invalids.push(x);
            }
        }
    ...
    if (confirmMode) {
            ...
            for (var x = 0; x < vitems.length; x++) {
                ...
                if (vi.length > 1) {
                    for (var y = 1; y < vi.length; y++) {
                        var el = vi[y];
                        if (isDate(el)) {
                            newA.itemProperties.item(dueProperty).Value = el;
                        } else {
                            cats += el + ";";
                        }
                    }
        ...
        if (icount > 0 && !passedLine) {
            Ext.getCmp("cinb").setText(txtInbox + "&nbsp;(" + countInboxItems() + ")");
            Ext.getCmp("cinb").getEl().highlight("#ff0000", { attr: "background-color", endColor: "ffffff", easing: 'easeIn', duration: 2 });
        }
    }
...
//save collect definition
    if (thisDefinition > 0) {
        var choice = confirm(txtColPrmReplDef);
        if (!choice) {
            return;
        }
...
function getSavedDefs() { //refresh bottom panel with saved import definitions
...
//delete a saved definition
    ...
    if (noalert == null || noalert == false) {
        if (confirm(txtColSvdDefsRmQ + ":" + tnr + " ?") == false) {
            return;
        }
    }
...
//open a saved definition
    ...
    var r = impSetStore.getAt(ix);
    try {
        var fid = r.get("folderid");
        var fn = r.get("name");
        var fd = NSpace.GetFolderFromID(fid);
    } catch (e) {
        alert(txtColPrmNonExFld);
        return;
    }
    ...
    if (editor == true) { //open in editor
        collectOLFolder(fid, fd, arc, rem, cre, cim, dim, don, dup, flt, sflt, cflt);
        thisDefinition = id;
        updateColStatus();
    } else { //run import definition
        try {
            if (notEmpty(flt)) {
                var tc = fd.Items.Restrict("@SQL=" + flt);
            } else {
                var tc = fd.Items;
            }
        } catch (e) {
            alert(txtColPrmDefEr);
            return;
        }
...
//If duplicate is found the one last changed is kept
    ...
    if (f.Count > 1) {
        ...
        var checkedItem = 0;
        var otherItem = 0;
        for (var x = 1; x <= f.Count; x++) { //loop through items. Find original item and one more
            if (it.EntryID == f(x).EntryID) {
                checkedItem = x;
            } else {
                if (otherItem == 0) {
                    otherItem = x;
        ...
        var dChecked = itDate.clone();
        var dOther = new Date(f(otherItem).LastModificationTime);
        if (dChecked < dOther) {
            var olderItem = f(checkedItem);
            oParent = itParent;
            var ddo = dChecked.clone();
            var newerItem = f(otherItem);
            nParent = f(otherItem).Parent;
            var ddn = dOther.clone();
        } else {
            var newerItem = f(checkedItem);
            nParent = itParent;
            var ddn = dChecked.clone();
            var olderItem = f(otherItem);
            oParent = f(otherItem).Parent;
            var ddo = dOther.clone();
        }
...
function importTextAdvanced(confirmMode, passedText, passedCategory) {
    ...
    if (confirmMode) {
        ...
        if (cp > 0) {
            countTxt += cp + " Contacts\n";
        }
        if (ca > 0) {
            countTxt += ca + " Appointments\n";
        }
        if (cn > 0) {
            countTxt += cn + " Notes\n";
        }
        ...
        if (jcalendarItems == null) {
            jcalendarItems = calendarItems;
        }
        if (jcontactItems == null) {
            jcontactItems = contactItems;
        }
        //add items
        if (vitems.length > 0) {
            for (var x = 0; x < vitems.length; x++) {
                ...
                if (vi.type == "a") {
                    ...
                    newA = jcontactItems.Items.Add();
                    coCount++;
                } else {
                    newA = createActionOL(vi.desc);
                    icount++;
                }
        ...
        var counters = "";
        if (icount > 0) {
            counters += icount + " " + txtCbActions;
        }
        if (coCount > 0) {
            counters += "<br>" + coCount + " " + txtContacts;
        }
        if (caCount > 0) {
            counters += "<br>" + caCount + " Appointments";
        }
        if (nCount > 0) {
            counters += "<br>" + nCount + " Notes";
        }
...
function toggleAdvancedMode() {
    ...
    if (jello.collectAdvancedMode) {
        Ext.getCmp("cjoin").show();
    } else {
        Ext.getCmp("cjoin").hide();
    }
    jese.saveCurrent();
}

function doTextCollect() { //run the text collect process
    Ext.getCmp("cotags").focus();
    if (jello.collectAdvancedMode == true || jello.collectAdvancedMode != 0) {
        var c = importTextAdvanced(true);
        if (c == true) {
            importTextAdvanced(false);
        }
    } else {
