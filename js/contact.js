...
//delegate item to contact, add to @waiting and archive
    ...
    try {
        Ext.getCmp("thecontactform").destroy();
    } catch (e) {
    }
...
//shortcut keys
    var map = new Ext.KeyMap('thecontactform', {
        ...
        fn: function() {
            ...
            try {
                Ext.getCmp("theactionform").setActive(true);
            } catch (e) {
            }
        },
        scope: this
...
//contactList array must be used for storing contacts
    ...
    if (iits.Count == 0) {
        ret = "<tr><td colspan=2 class=formconlistchars><i>" + txtConNoMatch + "</i></td></tr>";
    }
...
function addContactsToItems(recs, cons) {
    ...
    var conlist = "";
    if (cons.length == 0) {
        var noco = confirm(txtDelgNoCons);
        if (noco == false) {
            return true;
        }
    }
...
//attach a file to active action
    ...
    if (notEmpty(v)) {
        ...
        try {
            if (!linkOnly) {
                var att = it.Attachments.Add(v);
            } else {
                var att = it.Attachments.Add(v, 6);
            }
        } catch (e) {
            alert(txtInvalid);
