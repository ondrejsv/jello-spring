    ...
    if (c > 0) {
        ...
        var addWidget = function(rec) {
            ...
            if (!notEmpty(wId) || wId == "undefined") {
                ...
                if (winfo[2] && winfo[3] == false) {
                    tls = "tools:[tSet,tRefresh],";
                }
                if (wtitle == txtWidTJPostit) {
                    tls = "tools:[tClear,tCol,tSet,tClose],";
                }
                //create the widget
                if (jello.startup == 3 && firstLoadHome) {
                    iswcol = "true";
                }
                if (iswcol == "false") {
                ...
                try {
                    var textW = Ext.util.JSON.decode(newW);
                } catch (e) {
                    if (tls.substr(tls.length - 1, 1) == ",") {
                        tls = tls.substr(0, tls.length - 1);
                    }
...
function checkWidgets() { //check for widget records
    status = txtMsgStChecking;
    var lcol = 1;
    if (jello.widgetColumns == 1) {
        lcol = 0;
    }
    var c = widgetStore.getCount();
...
//render a widget list
    ...
    var wrtWidg = function(rec) {
        ...
        if (!notEmpty(wic)) {
            wic = "icon_extension.gif";
        }
        if (rec.get("unique") && widgetExists(wiID)) {
            torender = false;
        }
        //Outlook 2010 can have native views only in IE or hta mode
        if (OLversion > 13 && conStatus == "Outlook" && wiID == "OLW") {
            torender = false;
        }
...
function saveWidgetSettings(id) { //save the custom settings for a widget
    try {
    ...
    if (customizingWidget == null || r == null) {
        alert(txtInvalid);
        return;
    }
...
function resizePortlet(p, aw, ah, w, h) {
    ...
    for (var x = 0; x < p.items.length; x++) {
        ...
        setTimeout(function() {
            ...
            var marg = 0;
            if (OLversion >= 12 && jello.addSpaceOlView == true) {
                marg = OL2007VwMargin;
            }
            try {
                var ov = document.getElementById("olv:" + wid);
                ov.style.height = ht - (29 + marg);
            } catch (e) {
            }
...
function getWidgetHeight(id) { //return saved height of widget
    var ix = widgetStore.find("widgetID", new RegExp("^" + id + "$"));
    var r = widgetStore.getAt(ix);
    if (r == null) {
    ...
    var wtype = r.get("id");
    if (wtype == "IMP" || wtype == "WFL") {
        return null;
    }
    var wht = r.get("height");
    if (wht == 0 || !notEmpty(wht)) {
        wht = defaultWidgetHeight;
    }
    return wht;
}
...
//set home screen columns
    ...
    for (x = 0; x < 10; x++) {
        try {
            document.getElementById(f + x).style.visibility = "hidden";
        } catch (e) {
        }
    }
...
function setWidgetColumns(b) {
    ...
    for (x = 0; x < 10; x++) {
        try {
            document.getElementById(f + x).style.visibility = "visible";
        } catch (e) {
        }
    }
    if (b == "cancel") {
...
function resetWidgets(bup) { //reset widget settings and reload
    if (bup) {
        backupSettings();
    }
    jello.widgets = [];
...
function doWidgetRefresh(panel, pid) {
    if (panel.collapsed && panel.title != txtWidTImportant) {
        return;
    }
    ...
    } else if (nm == txtWidTJTagAct) { //var ret=eval(fun.replace("()","("+pid+",null,'')"));
        pHome();
        return;
    } else {
