...
var emailColumns = [
    'EntryID', 'ReceivedTime', 'Subject', 'Importance', 'SenderName', 'To', 'CC', 'urn:schemas:httpmail:textdescription',
    'urn:schemas:httpmail:hasattachment', 'TaskDueDate', 'UnRead', "MessageClass", "Sensitivity", "Categories"
];
var taskColumns = [
    'EntryID', 'CreationTime', 'Subject', 'Importance', '', '', '', 'urn:schemas:httpmail:textdescription',
    'urn:schemas:httpmail:hasattachment', 'DueDate', 'UnRead', "MessageClass", "Sensitivity", "Categories"
];
var emailStoreObjNames = [
    'entryID', 'created', 'subject', 'importance', 'sender', 'to', 'cc', 'body', 'attachment',
    'due', 'unread', 'iclass', 'sensitivity', 'tags'
];
// filed to sort on for various item types
var sortTables = [
    "ReceivedTime", "Start", "FullName",
...
//clear=true clear store before adding new records
    ...
    if (OLversion >= 12 && useTables && (theFolder.DefaultItemType == 0 || theFolder.DefaultItemType == 3)) {
        return getInboxTableItems(cmd, istore, olFolder, dasl, page, clear);
    }
...
//add actions
    if (cmd == 1) {
        addToStore(iits, istore, page, clear, false);
    }

    return counter;
}

function addToStore(olitms, istore, page, clear, updateButtons) { //add outlook item set in store
...
// if olitems is not an Items collection then tables
    ...
    try {
        var g = Ext.getCmp("igrid");
        g.hide();
    } catch (e) {
    }
    if (isInbox == true && page == 1 && updateButtons == true) {
        pInbox();
        return;
    }
    if (olitms == null) {
        olitms = lastInboxItems;
        istore = lastListStore;
    } else if (typeof(Ext.getCmp("gotoStore")) == "undefined") {
        lastInboxItems = olitms;
        lastListStore = istore;
    }
    if (olitms.Class != 16 && OLversion >= 12 && useTables) {
        return addTableItemsToStore(olitms, istore, page, clear, updateButtons);
    }
...
//clear store
    ...
    if (page > -999) {
        ...
        if (page > 0) {
            ...
            lStart = 0;
            lEnd = (allCount > psize ? psize : allCount);
        }
        try {
            Ext.getCmp("inboxgotopage").setRawValue("" + page);
        } catch (e) {
        }
        if (lStart < 1) {
            lStart = 1;
    ...
    lPage = page;
    if (isInbox == true) {
        allCount += inboxTasksCount();
    }


    if (updateButtons == true) {
        updatePagingToolbar(istore, allCount);
    }
    try {
        g.show();
...
//add actions
    if (cmd == 1) {
        addTableItemsToStore(iits, istore, page, clear, false, true);
    }

    return counter;
}

function addTableItemsToStore(olitms, istore, page, clear, updateButtons, setlastfolder, isInboxStore) { //add outlook item set in store
//alert(isInbox +" "+ page  +" "+ updateButtons);
    ...
    if (isInbox == true && page == 1 && updateButtons == true && isInboxStore) {
        pInbox();
        return;
    }
    if (olitms == null) {
...
//render the inbox items paging toolbar
    try {
        ...
        if (isInbox == false) {
            if (psize * lPage >= counter) {
                showOlder = false;
            }
        } else {
            if (psize * lPage >= (counter - inboxTasksCount())) {
                showOlder = false;
            }
        }

        if (showNewer == true) {
            Ext.getCmp("inewer").enable();
        } else {
            Ext.getCmp("inewer").disable();
        }
        if (showOlder == true) {
            Ext.getCmp("iolder").enable();
        } else {
            Ext.getCmp("iolder").disable();
        }
        if (isInbox == true) {
            var ibc = inboxTasksCount();
            lStart = lStart + ibc;
            lEnd = lEnd + ibc;
        }
        var ccls = lStart;
        ...
        if (customCounterLabel == null) {
            customCounterLabel = txtDisplay + " " + ccls + "-" + (lEnd);
        }
//newfun
...
//	lastInboxItems=its;
        ...
        Ext.getCmp("ifilter").setText("Filter");
        pInbox(lIbFolder);
    }
}
...
//Trying to make user experience more complete, get sender and body information with one click and one security alert for the whole list
    var extractOL = function(r) {
        ...
        if (jello.mailPreviewHTML == true) {
            ...
            prw = cleanBody(it.HTMLBody);
        }

        r.set("body", "<div>" + prw + "</div>");
        if (it.Class == 43) {
            r.set("sender", it.SenderName);
            r.set("to", it.To);
        } else {
            prw = it.Body;
    ...
    for (n = 0; n < data.length; n++) {
        ...
        if (t.DefaultItemType == 0) {
            if (typeof(makeTask) != "undefined" && makeTask != null && makeTask == true) {
                ...
                ta.push({ folder: t });
                try {
                    if (it.Categories == "") it.Categories = "!Next";
                } catch (e) {
                }
                tasks.push(archiveItem(it, cit, ta, noRecord, true, true)[0]);
            } else
                try {
                    moveMailTo(it, t, cit, noRecord);
                } catch (e) {
                    alert(txtMsgNoMoveErr);
                    continue;
                }
    ...
    for (n = 0; n < data.length; n++) {
        ...
        if (t == null || t.DefaultItemType == 0) {
            ...
            if (actions.del == true) {
                ...
                try {
                    if (t != null)moveMailTo(it, t, cit, noRecord);
                } catch (e) {
                    alert(txtMsgNoMoveErr);
                    continue;
                }
            if (t != null && actions.copy != true && !noRecord) {
                try {
                    updateRecordItem(cit, true);
                } catch (e) {
                }
...
function archiveSelecions(param) { //helper archive function to run from hyperlinks
...
function updateInboxRecord(rec) { //update grid record after change
    try {
...
function openInboxItem(id) { //open inbox item for editing
    if (typeof(id) == "undefined" || id == null || id == "") {
        ...
        var xid = id;
    try {
        var it = NSpace.GetItemFromID(xid);
    } catch (e) {
...
//get custom due field value for emails. Create if not exists
    ...
    try {
        ...
        if (OLversion < 12 || jello.useOLDue == false) it.UserProperties.Add("jdue", 1, true);
    }
...
// convert if custom field already in use
        ...
        if (jn != "" && isDate(jn)) {
            ...
            try {
                it.UserProperties.Item("jdue").Value = "";
            } catch (e) {
            }
        } else {
            jn = dn;
...
function setJDueProperty(it, due) {
    if (OLversion < 12 || jello.useOLDue == false) {
        ...
        try {
            ...
            it.UserProperties.Add("jdue", 1, true);
            it.UserProperties.Item("jdue").Value = due;
        }
    } else {
...
//return array:1.Status success/fail, 2.Created Task ref, 3.Archived Item ref
    ...
    if (it.Class != 44) {
        ...
        if (notEmpty(cats)) {
            cats = checkForNotExistingTags(cats);
        } else {
            createTask = false;
        }
    }
    ...
    if (createTask) {
        if (ibod.substr(0, 9) == "<secured>") {
            ...
            var tl = toplayer.innerHTML;
            toplayer.innerHTML = ibod;
            ibod = toplayer.innerText;
            toplayer.innerHTML = tl;
        }
    }
    ...
    if (archiveIt) {
        ...
        if (copyToo.length > 0) {
            ...
            if (it.Parent == af) {
                ...
                try {
                    finalItem = it.Move(af);
                } catch (e) {
                    alert(txtArchiveCannot);
                    return;
                }
            }
        }
    ...
    if ((notEmpty(cats) || isDate(idue)) && createTask) {
        ...
        newA = createActionOL(finalItem.Subject);
        ...
        if (notEmpty(ibod)) {
            ...
            if (jello.autoUpdateTaskNotes == false) {
                getJNotesProperty(newA);
                setJNotesProperty(newA, ibod);
            }
        }
...
function getDefaultArcFolder() { //return the default archive folder if not exists prompt for it
...
//if no archive folder defined define one
    var choice = false;
    if (choice = confirm(txtPrmptNoArcFldr) == false) {
        return;
    }
    var t = NSpace.PickFolder();
    if (t != null) {
        if (t.EntryID == jello.inboxFolder) {
            alert(txtMsgNoSystemFolder);
            return;
        }
        if (t.DefaultItemType == 0) {
...
function countInboxItems() { //inbox counter
    var counter = 0;
    try {
        var iF = NSpace.GetFolderFromID(jello.actionFolder).Items;
...
//Add a new item of the default type
    ...
    if (isInbox != null) {
        if (isInbox || (lIbFolder == null)) {
            var nit = NSpace.GetFolderFromID(jello.inboxFolder).Items.Add();
        } else {
            if (folderOnView == 0) {
                folderOnView = NSpace.GetFolderFromID(jello.inboxFolder);
            }
            nit = folderOnView.Items.Add();
        }
    } else {
    ...
    if (it.Class > 0) {
        ...
        if (c.length > 0) {
            for (var x = 0; x < c.length; x++) {
                ...
                if (notEmpty(af)) {
                    if (!valExistsInArray(af, afolder)) {
                        afolder.push(af);
                    }
                } else {
                    ...
                    if (notEmpty(daf)) {
                        if (!valExistsInArray(daf, afolder)) {
                            afolder.push(daf);
                        }
                    }
                }
...
//main.innerHTML="";
    ...
    { //inbox view
        ...
        var dasl = getInboxTaskDASL();
        if (jello.noUseMail == 1 || jello.noUseMail == "1") {
            showNoMailInbox(dasl);
            return;
        }
...
//    counter+=getInboxItems(1,listStore,folder,folder.CurrentView.Filter,1,false);
                    counter += getInboxItems(1, listStore, folder, null, 1, false);
                } catch (e) {
                }
...
//does user want an native Outlook view?
        ...
        if (OLversion > 13 && conStatus == "Outlook") {
            nativeView = false;
        }
        if (nativeView) {
            pOLInbox(folder);
...
//counter=getInboxItems(1,listStore,folder,null,1,true);
    ...
    fprops = "";
    if (!isInbox) {
        fprops = folderOnView.EntryID;
    } else {
        fprops = jello.inboxFolder;
    }
...
//if( !isInbox)
    ...
    var inboxChosenView = new Ext.grid.GroupingView({
        forceFit: true,
    ...
    if (jello.inboxGroupOn == false) {
        inboxChosenView = null;
    }
...
//ctrl+alt+J/K moves to previous/next page
    var gmap4 = new Ext.KeyMap('igrid', {
        key: 'k',
        fn: function() {
            if (Ext.getCmp("iolder").disabled == false) {
                addToStore(null, null, (lPage + 1), true, true);
            }
        },
        stopEvent: true,
        ctrl: true,
    ...
    var gmap5 = new Ext.KeyMap('igrid', {
        key: 'j',
        fn: function() {
            if (Ext.getCmp("inewer").disabled == false) {
                addToStore(null, null, (lPage - 1), true, true);
            }
        },
        stopEvent: true,
...
function updateMailItemDisplay(r) {
    try {
        ...
        r.endEdit();
        var gb = r.get("body");
        if (gb.substr(0, 9) != "<secured>" && jello.markReadfromJello == true) { //if user can read preview update read property of message
            it.UnRead = false;
            it.Save();
        }
    } catch (e) {
...
function getNewInboxItems(keypress) {
    ...
    if (OLversion >= 12 && useTables) {
        getNewInboxTableItems(keypress);
        return;
    }
    status = "Updating...";
    iboxTimerCount++;
    try {
        if (isInbox == false || getActiveGrid().getId() != "igrid") {
    ...
    try {
        ...
        for (var x = 1; x <= nic; x++) {
            try {
                var ar = new actionObject(iits.Item(x));
            } catch (e) {
                nic--;
                continue;
            }
...
function getNewInboxTableItems(keypress) {
    ...
    var g = getActiveGrid();
    try {
        if (isInbox == false || g.getId() != "igrid" || lPage != 1) {
...
function mailContextMenu(rec, id, it, inlist) {
    ...
    try {
        Ext.getCmp("inrclickmenu").destroy();
    } catch (e) {
    }
    try {
        Ext.getCmp("flagmenu").destroy();
    } catch (e) {
    }
...
function updateOLViewCounter() {
    try {
        updateCounter(olnative.ItemCount);
    } catch (e) {
    }
...
//manage archive for a list of items
    var ic = aItems.length;
    var cTasks = 0;
    var aCounter = 0;
    var archivers = new Array();
    ...
    for (var x = 0; x < ic; x++) {
        ...
        if (otype == 53 || otype == 54 || otype == 181 || otype == 55 || otype == 56 || otype == 57) {
            otype = 1000;
        }
        switch (otype) {
        case 43:        // mail
        ...
        if (arcfolder != null) {
            if (arcfolder.length == 1) { //one archive folder. Do archive
                ...
                if (!valExistsInArray(af.EntryID, archivers)) {
                    archivers.push(af.EntryID);
                }
                ...
                for (var x = 0; x < arcfolder.length; x++) {
                    ...
                    if (!valExistsInArray(cf.EntryID, archivers)) {
                        archivers.push(cf.EntryID);
                    }
                }
                var ait = archiveItem(it, rec, copyToo, noRec, archiveIt, createTask);
            }
    ...
    if (aCounter == 1) {
        ...
        if (aai.Parent.EntryID == jello.actionFolder) {
            msg = txtTkFiledIfo;
        }
        if (ait[0].task != null) {
            msg += "<br>" + txtAndCrThisSub + " <a class=jellolinktop onclick=scAction('" + ait[0].task.EntryID + "')>" + txtAction + "</a>";
        }
        return msg;
    }
    if (aCounter > 0) {
        arcAll = archiversList(archivers);
        var ams = txtProcessed;
        if (arcAll.length > 1 && cTasks == 0) {
            ams = txtArcToIfo + " " + arcAll;
        }
        if (cTasks > 0 && (aCounter - cTasks) > 0) {
            ams = cTasks + " " + txtProcessed + " " + (aCounter - cTasks) + " " + txtArcToIfo + " " + arcAll;
        }
...
function setAndCheckArcFolder(id) { //return a valid outlook archive folder ot null
    var rt;
    try {
        rt = NSpace.GetFolderFromID(id);
    } catch (e) {
        rt = null;
    }
    return rt;
...
//handle due date of items based on choice
    ...
    if (b == "no") {
        ...
        if (it.Class != 48) {
            ...
            setJDueProperty(it, "1/1/4501");
            it.Save();
        } else {
            it.itemProperties.item(dueProperty).Value = "1/1/4501";
            it.Save();
        }
        var id = it.EntryID;
...
function regroupInbox(a, b) { //enable/disable grouping of items
