...
function updateFolderInfo(fid) {
    ...
    if (hasHomepage) {
        ret += "<b>" + txtInWVIsOn + "</b><br><br>";
    } else {
        if (!notEmpty(homepage)) {
            ret += txtInWVIsOff + "<br><br>";
        } else {
            ret += "<b>" + txtInWPOn + "</b> " + txtInWpOnWvOff + ". <a class=jellolinktop onclick=setWViewOn('" + fid + "');><b>" + txtInWpClOn + "</b></a><br><br>";
        }
    }
    if (notEmpty(homepage)) {
        ret += "<a class=jellolinktop href='" + homepage + "'>" + homepage + "</a><br>";
    }
    if (homepage.search("jello5.htm") > -1 && hasHomepage) {
        ret += "<br><span class=tagprj>" + txtInJ5On + "</span>";
        jelloOn = true;
    }
    if (homepage.search("jelloDash.htm") > -1 && hasHomepage) {
        ret += "<br><span class=tagcon>" + txtInJ4On + "</span>";
        jelloOn = true;
    }
    if (jelloOn) {
        ib.disable();
...
function installAtFolder(fun) {
    ...
    if (fun == 0) {
        ...
        try {
            ...
            alert(txtInInvalidSetFldr);
        }
        if (ff.WebViewURL != uri) {
            alert(txtInInvalidSetFldr);
        }
    }
...
function selectJouFolder() {
    ...
    if (ff.DefaultItemType != 4) {
        alert(txtJouFolderPrompt);
        return;
    }
    Ext.getCmp("fID").setValue(ff.EntryID);
...
function changeJournalLocation() {
    ...
    try {
        var theNewFolder = NSpace.GetFolderFromID(newFol);
    } catch (e) {
        alert(txtInvalid);
        return;
    }
    ...
    if (newName != jelloSettingsName) {
        nameChange = true;
    }
    if (newFol != ff) {
        folderChange = true;
    }
    if (newFol == journalItems.EntryID) {
        newFol = "";
    }
    ...
    if (toMove) {
        if (nameChange) {
            jese.systemJournalEntry.Subject = newName;
            jese.systemJournalEntry.Save();
        }
        if (folderChange) {
    ...
    for (var x = 0; x < olFolders.length; x++) {
        ...
        if (homepage.search("jello5.htm") > -1 && hasHomepage) {
            ...
            try {
                ...
                fd.WebViewURL = uri;
                fd.WebViewOn = true;
                if (fd.WebViewURL != uri) {
                    ret += "<tr><td width=40% height=30><img src=img\\delete.gif>&nbsp;<i>" + (x + 1) + ". " + fd + "</i></td><td><span class=tagprj>Unavailable</span></td><td></td></tr>";
                    noFlag = 1;
                }
                fd.WebViewURL = olduri;
                fd.WebViewOn = oldwv;
                if (noFlag == 0) {
                    ret += "<tr><td width=40% height=30><img src=img//folder.gif>" + (x + 1) + ". " + fd.FolderPath + "</td><td></td><td><a class=jelloformlink onclick=installJelloHere('" + fd.EntryID + "')>Install</a></td></tr>";
                }
            } catch (e) {
                ret += "<tr><td width=40% height=30><img src=img\\delete.gif>&nbsp;<i>" + (x + 1) + ". " + fd + "</i></td><td></td><td><span class=tagprj>Unavailable</span></td></tr>";
            }
