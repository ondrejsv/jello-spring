...
function jelloSettingsDefaults() {
    ...
    if (OLversion >= 12) {
        this.autoUpdateSecuredFields = true;
    } else {
        this.autoUpdateSecuredFields = false;
    }
...
//jello settings object
    ...
    this.save = function(ss) {
        //save setting string
        try {
            var test = this.systemJournalEntry.UserProperties("settingString").Value;
        } catch (e) {
            this.systemJournalEntry.UserProperties.Add("settingString", 1, 0);
        }
    ...
    this.getSettingString = function() {
        ...
        //var ss="";
        try {
            var ss = this.systemJournalEntry.UserProperties("settingString").Value;
            return ss;
        } catch (e) {
            return null;
        }
    ...
    if (!notEmpty(jelloSettingsFolder)) {
        jelloSettingsFolder = journalItems.EntryID;
    }
    ...
    try {
        ...
        for (var i = 1; i <= cStores.Count; i++) {
            try {
                var aCol = cStores(i);
                var cc = aCol.Folders.GetFirst();
                document.title = "Loading store " + aCol + "...";
            } catch (f) {
            }
        }
    ...
    if (j.Count == 0) {
        ...
        if (ja.UserProperties.Count == 0) {
            ja.UserProperties.Add("settingString", 1, 0);
        }
        ja.Save();
        ...
        if (this.systemJournalEntry.UserProperties.Count == 0) {
            this.journalEntryCreated = true;
        } else {
            if (!notEmpty(this.systemJournalEntry.UserProperties("settingString").Value)) {
                this.journalEntryCreated = true;
            }
        }
    }
    if (this.journalEntryCreated == true) {
...
function recordToJSON(vr, tr, vs) { //add an ext record to json obj variable
...
function syncStore(theStore, vr) { //synchronize store to variable
...
function init() {
    ...
    if (jello.appLanguage != "en") {
        setLang(jello.appLanguage);
    }
    setTimeout(function() {
        if (jese.journalEntryCreated) {
            Ext.info.msg("Jello Settings", "Jello Settings journal entry created");
        }
    }, 100);


    if (jello.useTables == 0 || jello.useTables == "0") {
        useTables = false;
    }
    if (OLversion >= 12 && useTables) {
        inboxTable = getTable(inboxItems, "");
    }
...
//
    if (tm == "1" || tm == 1) {
        Ext.util.CSS.swapStyleSheet('themecss', 'extjs\\resources\\css\\xtheme-gray.css');
    }
    if (tm == "2" || tm == 2) {
        Ext.util.CSS.swapStyleSheet('themecss', 'extjs\\resources\\css\\xtheme-silverCherry.css');
    }
    if (tm == "3" || tm == 3) {
        Ext.util.CSS.swapStyleSheet('themecss', 'extjs\\resources\\css\\xtheme-midnight.css');
    }
    if (tm == "4" || tm == 4) {
        Ext.util.CSS.swapStyleSheet('themecss', 'extjs\\resources\\css\\xtheme-blueen.css');
    }
    if (tm == "5" || tm == 5) {
        Ext.util.CSS.swapStyleSheet('themecss', 'extjs\\resources\\css\\xtheme-yellow.css');
    }
    if (tm == "6" || tm == 6) {
        Ext.util.CSS.swapStyleSheet('themecss', 'extjs\\resources\\css\\xtheme-pink.css');
    }
}
...
//var a1=setAndCheckArcFolder(jello.inboxFolder);
    ...
    var msg = "";
    if (a2 == null) {
        jello.actionFolder = taskItems.EntryID();
        msg += "Action ";
    }
    if (a3 == null) {
        jello.contactFolder = contactItems.EntryID();
        msg += "Contacts ";
    }
    if (a4 == null) {
        jello.calendarFolder = calendarItems.EntryID();
        msg += "Calendar ";
    }
...
Ext.EventManager.on(window, 'load', function() {
    if (initOK) {
        ...
        if (jello.actionPreviewHeight > Ext.getBody().getViewSize().height) {
            jello.actionPreviewHeight = 200;
            jese.saveCurrent();
        };

        if (jello.theme != "0" && jello.theme != 0) {
            setTheme(jello.theme);
        }
        if (jello.firstRun == 1) {
        ...
        Ext.Ajax.request({
            ...
            success: function(a, b, c) {
                ...
                if (webJDversion != jelloVersion) {
                    ...
                    try {
                        Ext.info.msg("Application update", "A new Jello Dashboard version is available.<br>" + webJDversion + "<br><a href='http://www.jello-dashboard.net/download/' target='_blank'><b>Go to the downloads page</b></a>");
                    } catch (e) {
                    }
                    try {
                        Ext.getCmp("previewpanel").update(smallAbout());
                    ...
                    if (force) {
                        Ext.info.msg("Application update", "You have the latest version");
                    }
                }
            }
