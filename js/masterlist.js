...
//,{property:'private',value:!true}
    ...
    var masterTag = function(r) {
        ...
        try {
            var its = showContext(tname, 0, null, filter, null, true, true);
        } catch (e) {
        }
...
//render the subject line in grid
    ...
    if (tt == txtNew) {
        var tList = tagList("formtagdisplay", "...", alltag);
    }
...
function openMasterItem() { //open inbox item for editing
...
//preview & print masterlist
    ...
    var toPrint = function(rec) {
        ...
        if (lastTag != tg) {
            lastTag = tg;
            ret += "<tr><td colspan=2 valign=bottom class=printChars style=background:#FAEFC7;height:45px;><b>" + tg + "</b></td></tr>";
        }
        ...
        if (alltag.length > 1) {
            content += "&nbsp;<i>(" + alltag + ")</i></span>";
        }

        var nts = rec.get("body");
        var s = nts.search(txtTryToGetBody);
        if (notEmpty(nts) && s == -1) {
            content += "<br><span class=smallPchars>" + nts.substr(0, 500) + "</span>";
        } else {
