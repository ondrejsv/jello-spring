...
//check for existing previous version settings to migrate
    ...
    if (j.Count > 0) { //migrate contexts and projects
        var set4 = j(1);
        if (check == true) {
            ret = "<a class=jellolinktop onclick=settingsMigration()><font color=red>" + txtMigHomeMsg + "</font></a>";
            return ret;
        }
        ...
        if (choice) {
            migFols = migrateGetFolders(j4[69], (j4[6] == "1"));
        }
        ...
        for (var x = 0; x < cpList.length; x++) {
            ...
            if (notEmpty(nt)) {
                ...
                if (typeof(r) == "undefined") {
                    var newTag = createTag(tnt, true, impTags, false, true);
                    counter++;
                } else {
                    newTag = r.get("id");
                }
                try {
                    migrateCPProperties(tnt, newTag, folds, props);
                } catch (e) {
                }
            }
        }
            ...
            if (notEmpty(nt)) {
                ...
                if (typeof(r) == "undefined") {
                    if (tnt.substr(0, 1) == "~") {
                        ...
                        var newTag = createTag(tnt, true, arcTags, false, true);
                        counter++;
                    } else {
                        var newTag = createTag(tnt, true, 10, false, true);
                        counter++;
                    }
                } else {
                    newTag = r.get("id");
...
//basic contexts names
        ...
        if (r != null) {
            r.beginEdit();
            r.set("tag", Trim(j4[63]));
            r.endEdit();
            tagStore.clearFilter();
            syncStore(tagStore, "jello.tags");
        }
        var ix = tagStore.find("id", new RegExp("^9$"));
        var r = tagStore.getAt(ix);
        if (r != null) {
            r.beginEdit();
            r.set("tag", Trim(j4[64]));
            r.endEdit();
            tagStore.clearFilter();
            syncStore(tagStore, "jello.tags");
        }
        var ix = tagStore.find("id", new RegExp("^7$"));
        var r = tagStore.getAt(ix);
        if (r != null) {
            r.beginEdit();
            r.set("tag", Trim(j4[65]));
            r.endEdit();
            tagStore.clearFilter();
            syncStore(tagStore, "jello.tags");
        }
...
//loop migFols
        ...
        if (migFols.length > 0) {
            for (var x = 0; x < migFols.length; x++) {
                var f = null;
                try {
                    f = setAndCheckArcFolder(migFols[x].EntryID);
                } catch (e) {
                }
                if (f != null) {
...
//create tasks for all items existed in old version of Jello
    if (f.EntryID == jello.actionFolder) {
        return;
    }
    var actionF = setAndCheckArcFolder(jello.actionFolder);
    if (actionF == null) {
        status = txtMigStMsgInvAF;
        return;
    }
    var iF = f.Items;
    var ctx = tagName;
    ...
    for (var x = 1; x < counter; x++) {
        ...
        if (it.Class == 48) {
            ...
            try {
                var ait = archiveItem(it, null, null, true, false, true);
            } catch (e) {
            }
        }
    }
