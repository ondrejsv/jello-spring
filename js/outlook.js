...
function connect() {
    ...
    if (ol != false && typeof(ol) != "undefined") {
        conStatus = "Outlook";
        progressBar();
    } else {
        conStatus = null;
...
function olAddFolder(it, nd) {
    ...
    Ext.Msg.prompt(txtAddFolder, txtAddFolderPrmpt, function(bt, fname) {
        if (bt == "ok") {
            try {
                var t = NSpace.GetFolderFromID(addFolderTemp);
                t.Folders.Add(fname);
...
function isFolder(id) {
    try {
        var i = NSpace.GetFolderFromID(id);
        return true;
    } catch (e) {
        return false;
    }
...
// so the lookup can throw exception, put a try around it all
    try {
        ...
        if (d.Class == 48) {
            var tLinks = "";
            try {
                tLinks = d.UserProperties.Item("OLID").value;
            } catch (e) {
            }
            if (notEmpty(tLinks)) {
                var tl = tLinks.split(";");
                if (tl.length > 0) {
                    for (var x = 0; x < tl.length; x++) {
                        try {
                            ...
                            try {
                                it.UserProperties.Item("OLID").Value = "";
                                it.Save();
                            } catch (e) {
                            }
        ...
        alert("Could not perform selected action on Outlook item.");
        return true;
    };
    showOLViewCtl(true);
    updateInboxCount();
...
//open outlook folder in new window
    if (!notEmpty(id)) {
        id = jello.archiveFolder;
    }
    try {
        var fol = NSpace.GetFolderFromID(id);
    } catch (e) {
        alert(txtInvalid);
    }
...
//Outlook folder properties edit
    ...
    if (id == jello.inboxFolder) {
        finfo += "<span class=tagprj>Jello " + txtJInbFol + "</b>&nbsp;";
    }
    if (id == jello.actionFolder) {
        finfo += "<span class=tagprj>Jello " + txtJActFol + "</b>&nbsp;";
    }
    if (id == jello.archiveFolder) {
        finfo += "<span class=tagprj>Jello " + txtJArcFol + "&nbsp;";
    }
    if (id == jello.calendarFolder) {
        finfo += "<span class=tagprj>Jello " + txtJTicFol + "&nbsp;";
    }
    if (id == jelloSettingsFolder) {
        finfo += "<span class=tagprj><b>Jello " + txtJSetFol + "</b>&nbsp;";
    }
    if (shortcutExists("outlookView('" + id + "')")) {
        finfo += "<span class=tagnext>" + txtMsgShocCre;
    }
    finfo += "</P>";
    ...
    if (typeof(adolview) == "object") {
        adolview = adolview.Name;
    }
    if (!notEmpty(adolview)) {
        adolview = fol.Views.Item(1).Name;
    }
    ...
    if (oview == txtOLJView2) {
        showdolview = true;
    }

    try {
        var odesc = fol.Description;
    } catch (e) {
        var odesc = "";
    }
    try {
        var oentryid = fol.EntryID;
    } catch (e) {
        var oentryid = "None";
    }
            ...
            new Ext.form.ComboBox({
                ...
                listeners: {
                    select: function(cb, rec, idx) {
                        ...
                        if (showov) {
                            Ext.getCmp("olviewctl").show();
                        } else {
                            Ext.getCmp("olviewctl").hide();
                        }
...
//save Outlook properties form
    try {
        var id = Ext.getCmp("oid").getValue();
    } catch (e) {
        alert(txtInvalid);
        return;
    }
...
function getOLFolderCounters(id) {
    if (id.substr(0, 7) == "outlook") {
        id = id.replace("outlookView('", "");
        id = id.replace("')", "");
    }
    try {
...
//var hgt=Ext.getCmp("west-panel").getInnerHeight()-tp[1];
    ...
    var prp = 0;
    try {
        if (ff.DefaultItemType == 1 && OLversion > 11 && OLversion < 14) {
            prp = 80;
            Ext.getCmp("olview").setHeight(Ext.getCmp("olview").getHeight() - 65);
        }
    } catch (e) {
    }
//prp=jello.actionPreviewHeight;
    var hgt = (panelH - prp);
    var marg = 0;
    if (OLversion >= 11 && jello.addSpaceOlView == true) {
        marg = OL2007VwMargin;
    }
...
//restore outlook view defaults
    if (jello.hideOLPreviewPane == false || jello.hideOLPreviewPane == 0 || jello.hideOLPreviewPane == "0") {
        return;
    }
    try {
...
//Outlook folder view in Jello
    ...
    for (var x = 1; x <= folder.Views.Count; x++) {
        try {
            oViews.push(folder.Views.Item(x).Name);
        } catch (e) {
        }
    }
...
//in OL 2010 there is no filter property of OLViewControl Using Filters collection instead
    if (OLversion < 14) {
        if (notEmpty(olnative.Filter)) {
            olnative.Filter = "";
        }
    }

    if (OLversion <= 10) {
        Ext.getCmp("oprevw").hide();
    }
    updateOLViewCounter();
...
function showOLViewCtl(show) { //show/hide the outlook view control
    var ss = "hidden";
    if (show) {
        ss = "visible";
    }
    try {
        olnative.style.visibility = ss;
    } catch (e) {
    }
//hide/show widgets
    for (var x = 1; x < 20; x++) {
        try {
            var ov = document.getElementById("olv:" + x);
            ov.style.visibility = ss;
        } catch (e) {
        }
    }
}
...
//toggle outlook view preview on/off
    try {
        ...
        if (force != null) {
            ol.ActiveExplorer.ShowPane(3, force);
            return;
        }
...
function filterOLView() { //filter the outlook view control
    var press = Ext.getCmp("ifilter").pressed;
    if (press == false) {
        olnative.Filter = "";
        olnative.SaveView(olnative.View);
        return;
    }
...
//use saved view for outlook view control from outlook properties settings
    ...
    var vw = "";
    if (r == null) {
        vw = NSpace.GetFolderFromID(id).Views.Item(1).Name;
    } else {
        var vw = r.get("defolview");
    }
    if (typeof(vw) == "object") {
        vw = vw.Name;
    }
...
//Unset outlook folder to show the dashboard
    if (confirm("Unset Jello Dashboard from folder?") == false) {
        return;
    }
...
//get custom priority field value. Create if not exists
    try {
        ...
        try {
            ...
            if (it.Importance == 2) {
                it.UserProperties.Item("jpri").Value = 1;
                it.Save();
                jn = 1;
            }
        }
    ...
    if (folderListObjs.length == 0) {
        ...
        for (var i = 0; i < top.length; i++) {
            if (top[i].name.indexOf("Public") == 0)
                continue;
            try {
                top[i].name = NSpace.GetFolderFromID(top[i].id);
            } catch (e) {
                continue;
            }
...
function selFolder(boxTitle, funct, xdata, tagsToo) {
    ...
    if (xdata != null) {
        ...
        if (xdata.length == 1) {
            var cbox = new Ext.form.CheckboxGroup({
                ...
                xtype: 'checkboxgroup',
                columns: [
                    (txtTask.length + 1) / chrs, (txtReply.length + 1) / chrs,
    ...
    if (xdata != null && xdata.length == 1) {
        ...
        var ftagList = "";
        try {
            var it = NSpace.GetItemFromID(xdata[0][0]);
        } catch (e) {
            return;
        }
        ...
        var cbox2 = new Ext.form.ComboBox({
            ...
            listeners: {
                blur: function() {
                    var mx = Ext.getCmp("selFoldCombo");
                    quickAddTag("mtags", "mtagfield", 'mformtagdisplay', mx.folData[0][0]);
                },
                        ...
                        if (id.substr(0, 1) != "-") {
                            ...
                            if (typeof(facts) != "undefined") {
                                ...
                                for (var i = 0; i < factvals.length; i++) {
                                    ...
                                    switch (lbl) {
                                    case "ckTask":
                                        xacts.task = factvals[i].getValue();
                                        break;
                                    case "ckRepl":
                                        xacts.reply = factvals[i].getValue();
                                        break;
                                    case "ckAppt":
                                        xacts.appt = factvals[i].getValue();
                                        break;
                                    case "ckFwd":
                                        xacts.fwd = factvals[i].getValue();
                                        break;
                                    case "ckDel":
                                        xacts.del = factvals[i].getValue();
                                        break;
                                    case "ckCopy":
                                        xacts.copy = factvals[i].getValue();
                                        break;
                                    }
                                }
                            }
...
//Update outlook categories from jello contexts/projects (OL2007 only)
    ...
    if (OLversion > 11 && jello.updateOutlookCategories == true) {
        if (notEmpty(newName) == false) {
            newName = null;
        }
        var cExists = false;
        var cPos = 0;
        for (var x = 1; x < NSpace.Categories.Count; x++) {
            if (trimLow(NSpace.Categories.Item(x).Name) == trimLow(cpName)) {
                cExists = true;
                cPos = x;
            }
        }
        ...
        if (cExists == false && newName == null) {
            try {
                NSpace.Categories.Add(cpName);
                return true;
            } catch (e) {
                return false;
            }
        }
        if (cExists == true && newName != null) {
            try {
                NSpace.Categories.Item(cPos).Name = newName;
            } catch (e) {
            }
        }
        if (cExists == true && del == true) {
            try {
                NSpace.Categories.Remove(cPos);
            } catch (e) {
            }
        }
