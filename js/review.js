...
var tRvPath = {
    ...
    qtip: txtShowPaths,
    handler: function(e, target, panel) {
        if (jello.reviewPanelPaths == 1 || jello.reviewPanelPaths == "1") {
            jello.reviewPanelPaths = 0;
        } else {
            jello.reviewPanelPaths = 1;
        }
        jese.saveCurrent();
        Ext.getCmp("reviewacpanel").expand(true);
        updateReviewPanel();
...
//update review panel of sidebar
    if (typeof(tgName) == "number") {
        tgName = getTagName(tgName);
    }
    if (showZero) {
        btti = txtHideEmptyTags;
    }
    var showPaths = false;
    if (jello.reviewPanelPaths == 1 || jello.reviewPanelPaths == "1") {
        showPaths = true;
    ...
    var renderTag = function(r) {
        ...
        if (caption == getActionTagName()) {
            caption = txtNextActions;
        }
        ...
        var fmt = "";
        if (tid == ctid) {
            currentTag = (panelItems.length - 1);
            fmt = "class=tagprj";
        }
    ...
    if (showPaths) {
        reviewStore.each(pathTag);
    }

    if (showPaths) {
        reviewStore.sort("path", "ASC");
    } else {
...
function showZeroTags() { //update the panel to show/not show empty tags
...
//currentTag
    ...
    if (num > 0) {
        currentTag++;
        if ((panelItems.length - 1) < currentTag) {
            currentTag = 0;
        }
