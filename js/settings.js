        ...
        items: [
            ...
            new Ext.FormPanel({
                ...
                listeners: {
                    render: function() {
                        setTimeout(function() {
                            Ext.getCmp("ggrid").doLayout();
                            Ext.getCmp("ggui").doLayout();
                        }, 3);
                    }
                },
                frame: true,
...
//change a jello setting
    ...
    try {
        var ss = val.search("'");
        if (ss > 0) {
            alert(txtMsgSetQuote);
            el.setValue(val.replace(new RegExp("'", "g"), ""));
            el.focus();
            return;
        }
    } catch (e) {
...
//render a settings line type(0:checkbox 1:text 2:combo) vr(variable) cookie(setting id) last(value list for combos) txt(setting caption)
    ...
    if (type == 3) { //folder chooser
        var jf = "";
        try {
            jf = NSpace.GetFolderFromID(vr);
            var jfp = jf.FolderPath;
        } catch (e) {
            var jfp = txtUndefined;
        }
...
//general settings
    ...
    ret += "<tr><td class=printChars>" + txtSetIFSUse + ":</td><td class=printChars colspan=5><b>" + Ext.util.Format.htmlEncode(jelloSettingsName);
    + "</b> (" + txtSetIFLCh + " " + jMD.format('j F Y @H:i') + ")</a></td></tr>";
    ...
    if (webJDversion != jelloVersion && notEmpty(webJDversion)) {
        ret += "<tr><td class=printChars colspan=5><font size=3 color=red>A new Jello Dashboard version is available. " + webJDversion + " </font><a href='http://www.jello-dashboard.net/download/' target='_blank'><b>Go to the downloads page</b></a></td></tr>";
    }
    ret += "</table>";
    return ret;
}
...
//Update All+existing outlook categories from jello contexts/projects (OL2007 only)
    ...
    var doOLCUpdate = function(r) {
        ...
        var ret = false;
        if (itt == true) {
            ret = updateOLCategory(r.get("tag"));
        }
        if (ret == true) {
            uCount++;
        }
    ...
    if (uCount > 0) {
        alert(txtUpdateAllCatsInfo.replace("%1", uCount));
    }
}
...
function setJelloFolder(el) {
    ...
    if (t != null) {
        ...
        if (el.getId() == "contactFolder" && t.DefaultItemType != 2) {
            alert(txtPromptContactFolder);
            return false;
        }
        if (el.getId() == "calendarFolder" && t.DefaultItemType != 1) {
            alert(txtPromptCalendarFolder);
            return false;
        }
        if (el.getId() == "nonActionFolder" && t.DefaultItemType != 4) {
            alert(txtPromptJournalFolder);
            return false;
        }
...
function openSettingsFolder() { //open the settings folder
    var jF = jese.systemJournalEntry.Parent;
    var exp = jF.GetExplorer();
    exp.Display();
}
...
function fixOrphanTags() {
    ...
    if (oco > 0) {
		...
		*/
        ...
        alert(txtSINoFixedOrphans.replace("%1", okCount));
    }
...
//update view to show contents of settings set
    ...
    try {
        if (j.Count > 0) {
            ...
            var jjj = Ext.util.JSON.decode(stxt);
            var ret = "";
            for (prop in jjj) {
                var pp = jjj[prop];
                if (typeof(pp) == "object") {
                    ...
                    if (editMode) {
                        editLink = "<a id='setlink" + lid + "' class=jellolinktop style=text-decoration:underline; onclick=editsetng(" + lid + ",'" + prop + "');>";
                    }
        ...
        Ext.getCmp("setview").getEl().update("<img src=img//icon_alert.gif><br><h1>" + txtSetCannotView + "</h1>");
    }
}
...
//edit setting value
    var defval = "";
    if (typeof(rec) == "undefined" && typeof(fld) == "undefined") { //simple value
