...
function newShortcut(cmd, ndname, img) {
    ...
    if (shortcutExists(cc)) {
        exists = true;
    }

    if (exists == false) { //create shortcut
        if (!notEmpty(img)) {
            img = 'icon_link.gif';
        }
        if (cmd.substr(0, 11) == "outlookView") {
            try {
                var x = ndname.search("</span>");
                if (x > -1) {
                    ndname = ndname.substr(0, (x + 7));
                }
            } catch (e) {
            }
        }
...
//create shortcut from action list
    ...
    if (itype == "t") {
        ...
        try {
            ...
            if (isTag) {
                cmd = "showContext(" + id + ",1)";
            }
            if (prv) {
                img = "page_lock.gif";
            }
            if (arc) {
                img = "page_package.gif";
            } else {
                cmd = "showTagFolder(null," + id + ")";
            }
        } catch (e) {
        }
