...
//return an array of all tags
    ...
    for (var x = 0; x < tagStore.getCount(); x++) {
        ...
        if (notEmpty(tn) && isClosed != true) {
            arr.push(Ext.util.Format.htmlEncode(tn));
        }
    }
    globalTags = arr;
    tagStore.clearFilter();
...
function tagDescription(ctx) {
    try {
        ...
        var ret = "";
        if (notEmpty(fnotes)) {
            ret += "&nbsp;<span style=font-size:9px;color:black;><em>(" + fnotes.substr(0, 100) + ")</em></span>";
        }
...
function selectTagOnTree(r) { //select specific tag record on hierarchy
...
//open tag record in edit form
    ...
    if (tagsitems != null) {
        ...
        if (notEmpty(farchiveID)) {
            try {
                var farchive = NSpace.GetFolderFromID(farchiveID).FolderPath;
            } catch (e) {
                farchive = txtUndefined;
            }
        } else {
            try {
                var farchive = NSpace.GetFolderFromID(jello.archiveFolder).FolderPath;
            } catch (e) {
                var farchive = txtUndefined;
            }
        }
        var fsys = r.get("sys");
        var finfo = "";
        ...
        if (fsys == true) {
            finfo += "<span class=tagnext>" + txtSysTagIfo + "</span>";
        }
        if (farchived == true) {
            finfo += "<span class=tagnext>" + txtArchivedInfo + "</span>";
        }
        if (farchived == true) {
            finfo += "<span class=tagprj>" + txtProject + "</span>";
        }
        if (fisclosed == true) {
            finfo += "<span class=tagprj><b> " + txtCompletedInfo + " </b></span>";
        }
        var fttl = txtEditTag;
        var tchide = true;
    } else {
    ...
    try {
        var fparName = tagStore.getAt(tagStore.find("id", new RegExp("^" + fparent + "$"))).get("tag");
    } catch (e) {
        var fparName = "";
    }
    var fttype = txtTag;
    ...
    var simple = new Ext.FormPanel({
        ...
        items: [
            ...
            new Ext.form.ComboBox({
                ...
                forceSelection: true,
                listeners: {
                    beforeselect: function(cb, r, idx) {
                        if (idx == 2 && fid > 0) {
                            alert(txtAlrtCantChgFld);
                            return false;
                        }
                    }
                },
...
//delete a tag, move child tags to root, remove all those tags from actions, remove node, remove grid entry
    ...
    if (getTagSys(id) == true) {
        alert(txtMsgDeleteSysNo);
        return;
    }
...
//update outlook category list (ol07)
    ...
    if (node != null) { //if delete was called from a tree...
        node.remove();
        gridRemoveTag(id);
    }
    if (grid != null) {
        ...
        try {
            Ext.getCmp("tree").getNodeById(id).remove();
        } catch (e) {
        }
    }
...
//create a non existing tag from action combo
    if (isValidTag(tg) == false) {
        return;
    }
    var choice = false;
    if (choice = confirm(txtMsgNewTag.replace("%1", tg)) == false) {
        return null;
    }
...
//reload existing combos
    try {
        Ext.getCmp("itags").store.loadData(globalTags);
        Ext.getCmp("itags").collapse();
    } catch (e) {
    }
    try {
        Ext.getCmp("tags").store.loadData(globalTags);
        Ext.getCmp("tags").collapse();
    } catch (e) {
    }
    for (var x = 0; x < 20; x++) {
        var olitg = "itags:" + x;
        try {
            Ext.getCmp(olitg).store.loadData(globalTags);
            Ext.getCmp(olitg).collapse();
        } catch (e) {
        }
    }
...
//update tree
    ...
    try {
        if (nomsg != true) {
            Ext.info.msg(txtNewTagTtl, txtTag + ' ' + tg + ' ' + txtNewTagSucc);
        }
    } catch (e) {
    }
...
function isValidTag(s, noMsg) { //check new tag name for invalid characters
    if (s.charCodeAt(0) == 32) {
        alert("Tag name cannot begin with a space");
        return false;
    };
    try {
        var iv = true;
        for (var x = 0; x <= (s.length - 1); x++) {
            ...
            if (o == 38 || o == 39 || o == 126 || o == 44 || /* + o==43 ||*/ o == 59 || o == 42 /* brackets || o==91 || o==93*/) {
                if (noMsg != true) {
                    alert(txtMsgChar + " ( " + s.charAt(x) + " )");
                }
                iv = false;
            }
        }
...
//check a category series for non existing tags. Ask to create
    ...
    if (c.length > 0) {
        for (var x = 0; x < c.length; x++) {
            var t = Trim(c[x]);
            var tg = getTag(t);
            if (tg == null) { //non existing tag. Check with user
...
function tagManager() { //screen to manage all tags
...
function TagToolbar() { //render the tag manager toolbar
...
function tagSelected(btn, vl) { //tag manager toolbar actions
    if (btn.id == "adel") {
    ...
    for (var x = 0; x < sr.length; x++) {
        var t = sr[x].get("id");
        if (btn.id == "aedit" || btn == "aedit") {
            editTag(t);
            return;
        }

        if (btn.id == "adel") {
            deleteTag(t, null, Ext.getCmp("ggrid"));
        }
...
//return the full path in hierarchy of a tag in HTML
    ...
    if (justPath == null || justPath) {
        ret = "/" + Ext.util.Format.htmlEncode(ctx);
    } else {
        ret = "/" + ctx;
    }
    if (!notEmpty(parent)) {
    ...
    if (ctx == txtJObject && parent == 0) {
        ret = ctx;
        return ret;
    }
    if (parent == 0 || parent == "0") {
        if (justPath == null || justPath) {
            ret = "<a class=jellopathlink onclick='showTagFolder(null,null);'>" + txtJObject + "</a>" + ret;
            return ret;
        }
    }
...
//return custom icon control
    ...
    for (var x = 1; x <= inum; x++) {
        ...
        var sel = "";
        if (ii == fname + " (" + x + ").gif") {
            sel = "style='border:1px double black;background:yellow;'";
        }
    ...
    ret += "</table>";
    if (!notEmpty(ii)) {
        ret += "<br>Currently Using the default icon";
    } else {
...
function addJournalInTag(subj, otherTag) {
    ...
    try {
        var jf = NSpace.GetFolderFromID(jello.nonActionFolder);
    } catch (e) {
        alert(txtWdImpNoJou);
        return;
    }
    ...
    if (otherTag == null) {
        jit.itemProperties.item(catProperty).Value = lastContext;
    } else {
        jit.itemProperties.item(catProperty).Value = otherTag;
    }
...
function addReferenceItems(ctx, listStore, subj) {
    ...
    try {
        var jf = NSpace.GetFolderFromID(jello.nonActionFolder).Items;
    } catch (e) {
        return;
    }
...
function addTagToAL(tagname, olitem, rec, noRec) { //add a tag to an actionlist item
    if (olitem.Class == 44) {
...
//get tags from multiple items and return only the unique ones
    if (tags == null || tags == "" || typeof(tags) == "undefined") {
        return;
    }
    ...
    for (var x = 0; x < tag.length; x++) {
        ...
        var y = 0;
        do {
            if (trimLow(tag[x]) == trimLow(newtags[y])) {
                exists = true;
                break;
            }
            y++;
...
//----
    ...
    for (var x = 0; x < tag.length; x++) {
        if (notEmpty(tag[x])) {
            ...
            if (isValidTag(oo, true)) {
                ...
                var ixc = "style=cursor:hand;";
                var ix = "<span class=tagremove onclick=removeTag('" + ooo + "','" + id + "','" + tagdisp + "');>" + elmRemoveTag + "</span>&nbsp;"; //}else{var ix="";var ixc="";
                var tagClass = "tagcon";
                var theTag = getTag(oo);
                if (theTag == null) {
                    tagClass = "notag";
                    ix = "";
                    ixc = "style=cursor:hand; title='" + txtConvtoJTag + "' onclick=getUnknownTag('" + ooo + "','" + id + "');";
                } else {
                    if (theTag.get("isproject")) {
                        tagClass = "tagprj";
                    }
                }

                var tconame = oo;
                if (tconame.length > 16) {
                    tconame = tconame.substr(0, 16) + "...";
                }
    ...
    if (ctx == null && isnext == true) {
        ret += "<a class=tagnext style=cursor:auto;>" + txtNextAction + "</a><span class=tagremove onclick=removeTag('" + getActionTagName() + "','" + id + "','" + tagdisp + "');>" + elmRemoveTag + "</span>&nbsp;";
    }
    rt.push(ret);
    rt.push(isnext);
...
//quick add a tag from combobox into action form
    ...
    if (notEmpty(vl)) { //if there is no such tag create it
        if (getTag(vl) == null) {
            vl = createTagFromCombo(vl, tg);
        }
    }
    if (!notEmpty(vl)) {
        Ext.getCmp(tg).setValue("");
        return;
    }
    ...
    var tag = e.split(";");
    for (var x = 0; x < tag.length; x++) {
        if (trimLow(vl) == trimLow(tag[x])) {
            existsIn = true;
        }
        status = "Tagging...";
    }
    if (existsIn == false) {
        Ext.getCmp(tgfield).setValue(nv + ";" + vl);
    }
