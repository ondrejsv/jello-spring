...
function pTicklers(periodToStore, popup) {
    ...
    if (notEmpty(periodToStore)) {
        ...
        case "D":
            thisRangeStart = new Date();
            thisRangeEnd = new Date();
            break;
        case "W":
            var now = new Date();
...
new Ext.form.Label({
  ...
  */
    ...
    case "0":
        ticklerRangeSelected(Ext.getCmp("vday"));
        thisCalView = "vday";
        break;
    case "1":
        ticklerRangeSelected(Ext.getCmp("vweek"));
        thisCalView = "vweek";
        break;
    case "2":
        ticklerRangeSelected(Ext.getCmp("vmonth"));
        thisCalView = "vmonth";
        break;
    case "3":
        ticklerRangeSelected(Ext.getCmp("vagenda"));
        thisCalView = "vagenda";
        break;
    }

    if (notEmpty(thisCalView)) {
        Ext.getCmp(thisCalView).toggle(true);
    }
    updateTicklerTitle();
...
//add tickler items (26.appointments and 48.tasks and 43 messages)
    ...
    if (eID.Class == 48 || eID.Class == 43) { //due date for tasks
        var mid = 0;
        var did = 0;
        if (eID.Class == 43)
        ...
        if (eID.ReminderSet == true) {
            this.reminder = txtReminder + ":" + eID.ReminderTime;
        }
    } else { //due dates for appointments
        try {
            var t = new Date(eID.Start);
        } catch (e) {
        }
        var dd = DisplayDate(t);
        if (dd == null || dd == "") {
        ...
        if (eID.ReminderSet == true) {
            try {
                var rd = ol.Reminders.Item(this.subject);
                var nrd = new Date(rd.NextReminderDate);
                this.reminder = txtReminder + ":" + DisplayDate(nrd) + "@" + DisplayTime(nrd);
            } catch (e) {
                this.reminder = txtNoRemindata;
            }
        }
    }
...
//render the subject line in grid
    ...
    var don = r.get("done");
    if (cl > 99) {
        return "<b>" + Ext.util.Format.htmlEncode(v) + "</b>";
    }
    var rt = "";
...
//}catch(e){}
    ...
    if (cl == 48) {
        rt += NextActionToggleIcons(tg, id);
    }
    if (notEmpty(rmr)) {
        rt += "<img src=img\\clock.gif title='" + rmr + "'>&nbsp;";
    }
    if (rcr == true) {
        rt += "<img src=img\\recur.gif>&nbsp;";
    }
    rt += conicon + "&nbsp;" + subj + "&nbsp;" + tList[0];
    return rt;
}

function openTicklerItem(iid, otherGrid, oneitem) { //open tickler item for editing
    ...
    if (notEmpty(iid)) {
        ...
        if (it.Class == 26) {
            var stt = it.Start;
            var dtt = new Date(it.Start);
        } else {
            var stt = new Date(it.itemProperties.item(dueProperty).Value);
            var dtt = stt;
        }
        if (it.Class == 48) {
            scAction(it.EntryID);
        ...
        if (typeof(oneitem) != "undefined" && oneitem != null)
            ...
            if (notEmpty(otherGrid)) {
                var g = otherGrid.getSelectionModel();
            } else
...
function ticklerSelected(btn, vl, oneitem) {
    if (btn.id == "vnew") { //new appointment
        var iF = NSpace.GetFolderFromID(jello.calendarFolder).Items;
        var napp = iF.Add();
        napp.Display();
    }

    if (btn.id == "vedit" || btn.id == "avedit") { //edit selected item
        openTicklerItem(null, null, oneitem);
    }

    if (btn.id == "vdel" || btn.id == "vdone" || btn.id == "avdel" || btn.id == "avdone") { //delete selected item(s)
        loopSelectedTicklers(btn.id, oneitem);
    }

    if (btn.id == "vrefresh") { //refresh view
        try {
            var g = getActiveGrid();
            if (g.getId() == "tgrid") {
...
function loopSelectedTicklers(act, oneitem) { //loop selected ticklers (tasks/appts) and perform an action (delete, done)

    if (act == "vdel" || act == "avdel") {
        var amsg = txtMsgDelItem;
        var co = confirm(amsg + "\n" + txtActOnOccMsg);
        if (co == false) {
            return;
        }
    ...
    for (var x = 0; x < cl; x++) {
        ...
        if (it.Class == 26) { //appointment (check for occurrence. if exists use occurrence)
            if (it.IsRecurring) {
        ...
        if (act == "vdel" || act == "avdel") { //delete
            updateRecordItem(cit[x], true, it);
            it.Delete();
        }

        if (act == "vdone" || act == "avdone") { //mark completed
            completeTickler(it);
            if (jello.CalendarShowCompleted == "0" || jello.CalendarShowCompleted == false) {
                updateRecordItem(cit[x], true, it);
            } else {
                updateRecordItem(cit[x], false, it);
            }
        }
    }
}

function completeTickler(it) { //mark tickler as complete based on item's type
    if (it.Class == 26) { //appointment
...
//thisRangeEnd.setDate(thisRangeStart.getDate()+jello.agendaDays);
    ...
    if (btn.id == "vnext") { //next range
        isMove = true;
        if (thisCalView == "vday") {
            thisRangeStart.setDate((thisRangeStart.getDate() + 1));
            thisRangeEnd = thisRangeStart.clone();
        }
        if (thisCalView == "vweek") {
            thisRangeStart.setDate((thisRangeStart.getDate() + 7));
            thisRangeEnd.setDate((thisRangeEnd.getDate() + 7));
        }
        if (thisCalView == "vmonth") {
            thisRangeStart.setMonth((thisRangeStart.getMonth()) + 1);
            thisRangeEnd = thisRangeStart.clone();
    ...
    if (btn.id == "vprev") { //previous range
        isMove = true;
        if (thisCalView == "vday") {
            thisRangeStart.setDate((thisRangeStart.getDate() - 1));
            thisRangeEnd = thisRangeStart.clone();
...
function updateTicklerTitle(noUpdate) { //update screen title
    var ttl = DisplayDate(thisRangeStart) + " - " + DisplayDate(thisRangeEnd);
    var ddf = dayDifSimple(thisRangeEnd, thisRangeStart);
    if (ddf == 1) {
        ttl = " [" + thisRangeStart.toLocaleDateString() + "]";
    }
    if (ddf == 7) {
        ttl += " (" + txtWeek + " " + thisRangeStart.format('W') + ")";
    }
    if (ddf > 27 && ddf < 32) {
        ttl = thisRangeStart.format('F o');
    }
    if (noUpdate == true) {
        return ttl;
    } else {
...
//display of date and time for calendar views
    ...
    if (ad == false) {
        ...
        ret += " <span class=callday>[" + txtAllDay + "]</span>";
    }
    return ret;
}
...
//return day position based on today
    ...
    if (df > 1) {
        ret = "<span class=9futuregroup>" + txtFuture + "</span>";
    }
    if (df < 0) {
        ret = "<span class=1pastgroup>" + txtPastDue + "</span>";
    }
    if (df > 1 && duew == tdw) {
        ret = "<span class=3latergroup>" + txtLaterWeek + "</span>";
    }
    if (df > 1 && duew == (tdw + 1)) {
        ret = "<span class=4nextwgroup>" + txtNextWeek + "</span>";
    }
    if (df == 0) {
        ret = "<span class=2todaygroup>" + txtToday + "</span>";
    }
    if (df == 1) {
        ret = "<span class=2todaygroup>" + txtTomorrow + "</span>";
    }
    return ret;
}
...
function ticklerContextMenu(rec, id, it, inlist) {
    try {
        Ext.getCmp("atrclickmenu").destroy();
    } catch (e) {
    }
    var oneitem = new Array();
    oneitem.push(rec);
...
function addDuration(newRec) { //add duration of item to total duration. Exclude allday items
    if (newRec.data.allday == false) {
        totalDuration += newRec.data.duration;
    }
}