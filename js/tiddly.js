...
//update selected folder/file information
    ...
    if (notEmpty(fname)) {
        var twf = "";
        try {
            twf = fso.GetFile(fname);
            fileinfo = "File " + fname + " already exists.<br>Created on: " + twf.DateCreated + "\nFile Size:" + twf.Size + " bytes";
            Ext.getCmp("twgo").setText("<b>Replace Tiddlywiki File</b>");
        } catch (e) {
        }

    } else {
        fileinfo = "No file specified";
        Ext.getCmp("twgo").setDisabled(true);
    }
...
//var jpath=getAppPath().replace("jelloapp.html","")+"/res/";
    ...
    if (wikiFile.substr(wikiFile.length - 5, 5) != ".html" || wikiFile.substr(wikiFile.length - 4, 4) != ".htm") {
        wikiFile += ".html";
    }
    ...
    try {
        ...
        if (confirm("Replace existing file?") == false) {
            return;
        }
    } catch (e) {
    }
...
/*	if (mode==0)
    ...
    if (notEmpty(JDdata)) {
        ...
        if (o.length > 0) {
            for (var x = 0; x < o.length; x++) {
                ...
                for (var k = 0; k < line.length; k++) {
                    ...
                    if (ttags.search("jello") > -1 && ttags.search("template") > -1) {
                        systemFlag = true;
                    }
                    if (ttags.search("excludeLists") > -1) {
                        systemFlag = true;
                    }
                    if (ttags.search("jello") > -1 && ttags.search("systemConfig") > -1) {
                        systemFlag = true;
                    }

                    if (!systemFlag) {
                        s = line[k].search(" modified=");
                        if (s > -1) {
                            tmodif = line[k + 1];
                            k++;
                        }
                        s = line[k].search(" olid=");
                        if (s > -1) {
                            tolid = line[k + 1];
                            k++;
                        }
                        s = line[k].search(" tagid=");
                        if (s > -1) {
                            ttgid = line[k + 1];
                            k++;
                        }
                        s = line[k].search(" notes=");
                        if (s > -1) {
                            nfrom = line[k + 1];
                            k++;
                        }
                        ...
                        if (ls == -1) {
                            twbody = twbody.substr(7, (twbody.length) - 14);
                        } else {
                            twbody = "";
                        }
                    }
                ...
                if (!systemFlag) {
                    ...
                    var newRec = new wikiRecord(cr);
                    wikiStore.add(newRec);
                } else {
                    //system tiddlers
        ...
        for (var x = 0; x < wikiStore.getCount(); x++) {
            ...
            if (typeof(modif) != "undefined") {
                ...
                if ((isTg > -1 || isPj > -1) && isNA == -1 && isDO == -1 && tid == 0) {
                    ...
                    if (r == null) {
                        createTag(ttl, true, 0, false, false, pj);
                        newRecords++;
                    } else {
                        if (pj) {
        ...
        if (newRecords == 0 && changesFromTW == 0) {
            alert("No changes found to update to Outlook");
            return;
        }
        if (!confirm(ret + "\n" + newRecords + " New records will be imported and " + changesFromTW + " Records changes from Wiki")) {
            return;
        }
    ...
    for (var x = 1; x <= f.Count; x++) {
        ...
        if (ts.length > 0) {
            ...
            if (tagString.substr(tagString.length - 1, 1) == " ") {
        ...
        if (!notEmpty(itb)) {
            taskBody = getJNotesProperty(f(x));
            bodyFrom = "jd";
        }
...
//get tags
    var TWtags = "";
    var icreation = new Date().convertToYYYYMMDDHHMM();
    var imodif = new Date().convertToYYYYMMDDHHMM();
...
//--menus and defaults
    ...
    try {
        ...
        alert("Could not create file. Probably folder does not exist yet!");
        return;
    }
    var wss = TWheader + "\n" + otherTiddlers + "\n" + TWbody + "\n" + TWtags + "\n" + TWother + "\n" + TWfooter;
    twStream.Write(wss);
...
function passLastTW() {
    ...
    if (choice = confirm("Sync with last file used? " + lastf) == false) {
        return;
    }
...
//todo- check category string for newly added tags and add them into jello
    ...
    for (var x = 0; x < cts.length; x++) {
        ...
        if (cat != " " && notEmpty(cat)) {
            var r = getTag(cat);
            if (r == null) {
                createTag(cat, true, 0, false, false, false);
            }
        }
    }
