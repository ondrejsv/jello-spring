...
function todoSync() {
    ...
    try {
        txtfile = fso.GetFile(fname);
    } catch (e) {
        if (!confirm("File " + fname + " does not exist \n Do you want to create it?")) {
            return;
        } else {

            try {
                fso.CreateFolder(fso.GetFileFolder(fname));
            } catch (g) {
            }
            try {
                ...
                alert("Something went wrong. File could not be created!\n" + f.description);
                return;
            }
        }
    }
...
//add fields of txt line to table
    ...
    if (tLine == "" || tLine == " " || tLine == "  " || tLine.length < 5) {
        return;
    }
...
//return itemID if found
    ...
    try {
        ...
        counter = its.Count;
        if (counter > 0) {
            isThere = its(1).EntryID;
        }
    } catch (e) {
        alert("Cannot handle task string" + taskName);
    }
...
//handle an outlook item
    ...
    if (r == undefined || r == null) {
        ...
        if (ftags.substr(0, 7) == "{{{*}}}") {
            fpri = "A";
            ftags = ftags.substr(7, ftags.length);
        }
...
//handle each txt item
    ...
    if (r.get("done")) {
        tl = "x ";
        if (r.get("donedate") != null) {
            tl += new Date(r.get("donedate")).format("Y-m-d") + " ";
        }
    }
    if (r.get("priority") != "") {
        tl += "(" + r.get("priority") + ") ";
    ...
    if (r.get("created") != null) {
        tl += new Date(r.get("created")).format("Y-m-d") + " ";
    }
    tl += r.get("task") + " ";
    tl += r.get("tags") + " ";
