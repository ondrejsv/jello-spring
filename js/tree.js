...
//add root items
    for (var x = 0; x < tagStore.getCount(); x++) {
        ...
        if (pj) {
            nd.setText("<span class=nodeprj>" + nd.text + "</span>");
        }
        tgs.appendChild(nd);
    }
...
//custom icon overrides
    ...
    if (tp == true) {
        ic = imgPath + "list_components.gif";
    }
    if (tj == true) {
        ic = imgPath + "project.gif";
    }
    if (prv == true) {
        ic = imgPath + "page_lock.gif";
    }
    if (prv == true && !tp) {
        ic = imgPath + "folder_lock.gif";
    }
    if (tg == txtSystem) {
        ic = imgPath + "folder_lock.gif";
    }
    if (arc) {
        ic = imgPath + "page_package.gif";
    }
    if (cls) {
        ic = imgPath + "check.gif";
    }
    switch (id) {
    case 6:
        ic = imgPath + "isnext.gif";
        break;
    case 7:
        ic = imgPath + "list_review.gif";
        break;
    case 8:
        ic = imgPath + "list_waiting.gif";
        break;
    case 9:
        ic = imgPath + "list_someday.gif";
        break;
    }
...
function enumerateFoldersInStores() { // show the various PSTs in this Outlook Session
    // skip public folders
    var colStores;
    var oStore;
    var oRoot;
    ...
    for (var i = 1; i <= colStores.Count; i++) {
        try {
            ...
            Ext.info.msg(txtOLFolAccErr, txtOLFolAccErr2 + ' ' + colStores(i).Name);
        }
...
function enumerateFolders(oFolderID) { //**j5
...
/* search folder routine. Still needs work
  ...
  */
        ...
        for (var i = 1; i <= foldercount; i++) {
            ...
            var ff = new olFolderObject();
            ff.id = thisfold.EntryID;
            ff.name = jelloOLFName(thisfold, thisfold.EntryID);
            ff.icon = thisimg;
            ret.push(ff);
        }
...
//add a tag node
    ...
    if (nd == null) {
        editTag(null, false, 0);
        return;
    }

    if (parent > 0) {
        var ix = tagStore.find("id", new RegExp("^" + parent + "$"));
        var r = tagStore.getAt(ix);
        var isTag = r.get("istag");
        editTag(null, isTag, parent);
    } else {
        editTag(null, false, parent);
    }
}
...
function popUpClick(bt, nd) {
    ...
    if (bt == "insert-node") {
        ...
        if (data[0] == "olk") {
            olAddFolder(data[1], nd);
        }
    }

    if (bt == "ol-open") {
        if (data[0] == "olk") {
            OLFolderOpenNewWindow(data[1]);
        }
    }

    if (bt == "tg-icon") {
        if (data[0] == "tag") {
            customIconTagForm(data[1]);
        }
    }
    ...
    if (bt == "delete-node") {
        ...
        if (data[0] == "sho") {
            ...
            deleteShortcut(data[1], nd);
        }
        if (data[0] == "olk") {
            olDelFolder(data[1], nd);
        }
    }
...
function treeMenu(node, e) {
    ...
    c.contextNode = node;
    if (data[0] == "sho") {
        Ext.getCmp("tg-icon").hide();
        Ext.getCmp("ol-open").hide();
        Ext.getCmp("add-shcut").disable();
        Ext.getCmp("insert-node").disable();
        Ext.getCmp("edit-node").disable();
        Ext.getCmp("delete-node").enable();
        Ext.getCmp("tag-node").hide();
    }
    if (data[0] == "tag") {
        Ext.getCmp("tg-icon").show();
        Ext.getCmp("ol-open").hide();
        Ext.getCmp("add-shcut").enable();
        Ext.getCmp("insert-node").enable();
        Ext.getCmp("edit-node").enable();
        Ext.getCmp("delete-node").enable();
        Ext.getCmp("tag-node").enable();
    }
    if (data[0] == "olk") {
        Ext.getCmp("tg-icon").hide();
        Ext.getCmp("ol-open").show();
        Ext.getCmp("add-shcut").enable();
        Ext.getCmp("insert-node").enable();
        Ext.getCmp("edit-node").enable();
        Ext.getCmp("delete-node").enable();
    ...
    if (data[1] == "0") {
        ...
        Ext.getCmp("insert-node").disable();
        Ext.getCmp("edit-node").disable();
        Ext.getCmp("delete-node").disable();
        Ext.getCmp("add-shcut").disable();
        Ext.getCmp("tag-node").hide();
    }
    if (data[0] == "tag") {
...
//check for drag and drop
    ...
    if (obj.data.node.contains(obj.target)) {
        obj.cancel = true;
    }
    if (data[0] == "tag" && tdata[0] == "olk") {
        obj.cancel = true;
    }
    if (data[0] == "olk" && tdata[0] == "tag") {
        obj.cancel = true;
    }
    if (data[0] == "olk" && tdata[0] == "olk") {
        obj.cancel = true;
    }
    if (data[0] == "tag" && tdata[0] == "tag") {
        if (getTagType(data[1]) == false && getTagType(tdata[1]) == true) {
            obj.cancel = true;
        }
    }
    if (data[0] == "tag")
//{if (getTagSys(data[1])==true){obj.cancel=true;}}
        if (data[0] == "tag" && tdata[0] == "sho") {
            lastToShortcutParent = obj.data.node.parentNode;
        }
    if (data[0] == "olk" && tdata[0] == "sho") {
        lastToShortcutParent = obj.data.node.parentNode;
    }
}
...
function newShortCut(key, id, text, toNode, noDD) {
    if (key == "tag") {
        ...
        if (tpri && ttp) {
            thisimg = "page_lock.gif";
        }
        if (tpri && !ttp) {
            thisimg = "folder_lock.gif";
        }
        if (tarc) {
            thisimg = "page_package.gif";
        }
        if (notEmpty(tic)) {
            thisimg = tic;
        }
