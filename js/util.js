...
// the OL null date, 1/1/4501
    ...
    if (DisplayDate.arguments.length == 2)
        if (showOLNullDate != null) {
            doNull = showOLNullDate;
        }
    if (!notEmpty(t)) {
        return txtUndefined;
    }
    try {
        if (t.getYear() == 4501 && t.getMonth() == 0 && t.getDate() == 1 && !doNull) {
            return "";
        }
    } catch (e) {
    }
...
function DisplayAppDate(t, dayOnly) {
    ...
    if (t.getYear() == 4501 && t.getMonth() == 0 && t.getDate() == 1) {
        return "";
    }
    var tod = new Date();
    ...
    if (s < 1) {
        ss = "<span class=daypdue>";
    }
    if (s == 0) {
        ss = "<span class=daytoday>";
    }
    if (s == 1) {
        ss = "<span class=dayfuture>";
    }
    if (jello.dueDisplayDaysLeft == false) {
        if (jello.dueShowYear == true) {
            var yr = jello.dateSeperator + t.getFullYear();
        } else {
            var yr = "";
        }
        ...
        if (jello.dateFormat == "0" || jello.dateFormat == 0) {
            return ss + dow + " " + t.getDate() + jello.dateSeperator + (t.getMonth() + 1) + yr + "</span>";
        }
        if (jello.dateFormat == "2" || jello.dateFormat == 2) {
            return ss + dow + " " + yr + (t.getMonth() + 1) + jello.dateSeperator + t.getDate() + "</span>";
        }
        return ss + dow + " " + (t.getMonth() + 1) + jello.dateSeperator + t.getDate() + jello.dateSeperator + yr + "</span>";


        if (jello.dateFormat == "0" || jello.dateFormat == 0) {
            return ss + dayOfWeek(t.getDay(), 1) + " " + t.getDate() + jello.dateSeperator + (t.getMonth() + 1) + yr + "</span>";
        }
        if (jello.dateFormat == "2" || jello.dateFormat == 2) {
            return ss + yr + " " + t.getDate() + jello.dateSeperator + (t.getMonth() + 1) + jello.dateSeperator + dayOfWeek(t.getDay(), 1) + "</span>";
        }
...
//**j5
    ...
    if (jello.timeFormat == 0) {
        return t.getHours() + ":" + m;
    }
    if (t.getHours() > 12) {
        return (t.getHours() - 12) + ":" + m + ' PM';
    }
    if (t.getHours() == 12 && t.getMinutes() >= 0) {
        return (t.getHours()) + ":" + m + ' PM';
    }
...
//calc difference in days between two dates
    ...
    try {
        ...
        var dif = Math.ceil((day1.getTime() - day2.getTime()) / (one_day));
    } catch (e) {
    }
    return dif;
}
...
//if ((d1-d2)==0 && m1==m2 && y1==y2){xx=0;}
    if (xx == 0) {
        ret = "<span class=today>" + txtToday + "</span>&nbsp;&lt;&lt;";
        r = 0;
    }
    if (xx == 1) {
        ret = "<span class=info2>" + txtTomorrow + "</span>";
        r = 1;
    }
    if (xx > 1) {
        ret = "<span class=textNormal>" + (xx) + " " + txtDays + "</span>";
        r = xx;
    }
    if (xx < 0) {
        ret = "<span class=counter>" + (xx - (xx * 2)) + " " + txtdaysAgo + "</span>";
        r = -1;
    }
    if (no == true) {
        return r;
...
//check string for empty or null or space
    var ret = true;
    if (s == null || s == "" || s == " " || typeof(s) == "undefined" || s == "NaN") {
        ret = false;
    }
    return ret;
}
...
function rightClickItemMenu(e, row, g) {
    ...
    if (g.getId() == "grid" || iclass == 48) {
        ...
        try {
            it = NSpace.GetItemFromID(id);
            cmnu = mailContextMenu(rec, id, it, inlist);
        } catch (e) {
        }
    } else if (iclass == 26 || iclass == 53) {
        try {
            it = NSpace.GetItemFromID(id);
            cmnu = ticklerContextMenu(rec, id, it, inlist);
        } catch (e) {
        }
    }
    if (cmnu != null) {
...
function jelloDateFormatString(longyear) {
    ...
    if (jello.dateFormat == 0 || jello.dateFormat == "0") {
        fmt = "d" + dts + "m" + dts + year;
    }
    if (jello.dateFormat == 2 || jello.dateFormat == "2") {
        fmt = year + dts + "m" + dts + "d";
    }
    return fmt;
}
...
function createDonationAction() {
    try {
        ...
        dt.Importance = 0;
        if (r != null) {
            dt.itemProperties.item(catProperty).Value = r.get("tag");
        }
...
//returns a series of icons showing visually a percentage
    ...
    if (aall > 0 && count == 0) {
        return "<img src=img\\check.gif title='" + txtCompleted + "'>";
    }
    if (num == 0 && count == 0) {
        return "<img title='No actions' src=img\\clock.gif style='opacity:0.4;filter:alpha(opacity=40);'>";
    }
...
//calc difference in days between two dates
    ...
    try {
        ...
        var dif = Math.ceil((day1.getTime() - day2.getTime()) / (one_hour));
    } catch (e) {
    }
    return dif;
}
...
function checkForSmartDate(df, value) {
    ...
    if (typeof(df) != "undefined" && df != null)
        if (Ext.isDate(df.getValue())) { //normal datevalue
            return;
        } else { //other
...
function checkForSmartDatebase(df) {
    ...
    if (sword == "today" || sword == "now" || sword == "tod") {
        nd = new Date();
        nnd = nd.format(fmt);
    }
    if (sword == "tomorrow" || sword == "tom") {
        nd = new Date();
        nd.setDate(nd.getDate() + 1);
        nnd = nd.format(fmt);
    }
    if (sword == "next week" || sword == "nw") {
        nd = new Date();
        nd.setDate(nd.getDate() + 7);
        nnd = nd.format(fmt);
    }
    if (sword == "next month" || sword == "nm") {
        nd = new Date();
        nd.setDate(nd.getDate() + 30);
        nnd = nd.format(fmt);
    }
    if (sword == "next year" || sword == "ny") {
        nd = new Date();
    ...
    if (sword.substr(0, 3) == "in ") {
        ...
        try {
            ...
            if (typeof(num) == "number") {
                var itv = 1;
                if (trimLow(o[2].substr(0, 1)) == "m") {
                    itv = 30;
                }
                if (trimLow(o[2].substr(0, 1)) == "w") {
                    itv = 7;
                }
                if (trimLow(o[2].substr(0, 1)) == "y") {
                    itv = 365;
                }
                var pls = num * itv;
                nd = new Date();
                nd.setDate(nd.getDate() + pls);
                nnd = nd.format(fmt);
            }
        } catch (e) {
        }
...
//day names
    ...
    if (theday != null) {
        ...
        var tdd = nd.getDay();
        if (tdd < theday) {
            nd.setDate(nd.getDate() + (theday - tdd));
            nnd = nd.format(fmt);
        }
        if (tdd == theday) {
        ...
        if (tdd > theday) {
            nd.setDate(nd.getDate() + ((theday - tdd) + 7));
            nnd = nd.format(fmt);
        }
    }
