...
//try{
    ...
    if (jImportant.arguments.length > 1) { //customize
        if (!notEmpty(ws[0])) {
            ws[0] = "Yes";
            ws[1] = "Yes";
        }
...
//}catch(e){}
    ...
    setTimeout(function() {
        if (j[1] > 0) {
            ...
            try {
                pnl.setTitle(txtImportantTitle + " (" + j[1] + ")");
            } catch (e) {
            }
        }
    }, 3);
...
//past due
    if (ws[1] == "Yes") {
        if (ws[0] == "Yes") {
            ret += "<hr style='color:white;border-bottom:1px gray dotted;'>";
        }
    ...
    if (jello.migrated == false) {
        ...
        try {
            ...
            if (notEmpty(sm)) { //if user did not migrated from previous jello settings show these
...
//Settings:0=Widget Title, 1=HTML code
    ...
    if (wInfo.arguments.length > 1) { //customize
...
// 3 args for popups, if arg 3 =="", popup is for home page popup
    ...
    if (wTicklers.arguments.length == 2) { //customize
...
// plain popup not widget or home page popup
    ...
    ret = [
        ...
        "trackMouseOver:true,width:250,listeners:{mouseover: function(e){thisGrid='" + gid + "';},rowdblclick: function(g,row,e){openTicklerItem(null,g);}, cellcontextmenu: function (g, row, cell, e){rightClickItemMenu(e,row,g);}}})"
    ].join("");
...
function wTagActions(id) { //Tag Actions Widget
...
//set only title
    ...
    if (wTagActions.arguments.length == 2) { //customize
...
//set widget's contents
    ...
    if (wTagActions.arguments.length == 1) {
        try {
            wStore[id] = showContext(ws[0], 2);
        } catch (e) {
            ret = "new Ext.Panel({html:'&nbsp;ERROR!&nbsp;<b>" + ws[0] + "</b>" + txtTagNotFound + "'})";
            return ret;
        }
    }
    ...
    ret = [
        "new Ext.grid.GridPanel({store:wStore[" + id + "],id:'wgrid:" + id + "',",
        ...
        "},enableColumnMove:false,border:false})"
    ].join("");
...
//Settings:0=color, 1=font, 2=size;
    ...
    if (wPostIt.arguments.length == 2) { //customize
...
//set widget's contents
    ret = [
        "new Ext.form.TextArea({id: '" + wid + "',width:250, emptyText:'" + txtWdPoNoteHr + "', style:'background:" + ws[0] + ";font-family:" + ws[1] + ";font-size:" + ws[2] + ";',",
        "enableKeyEvents:true,listeners:{keyup: function(t, e){if(e.getKey()==e.ENTER){wPostItSave(t,'" + id + "');}}}})"
    ].join("");
...
//Settings:0=Outlook folder ID, 1=Outlook view
    ...
    if (wOutlookView.arguments.length == 2) { //customize
...
//create tags list
        ...
        for (var x = 0; x < tagStore.getCount(); x++) {
            ...
            if (notEmpty(tn) && isArc != true) {
                widgetTags.push(Ext.util.Format.htmlEncode(tn));
            }
        }
        widgetTags.sort();
...
//do not display preview pane button in old outlook versions and in HTA/IE mode
        if (!isActiveX && OLversion > 10) {
            olpre = "{icon:'img//olpreview.gif',cls:'x-btn-icon',id:'oprevw',hidden:" + isActiveX + ",tooltip:'" + txtOLPwPaneTgl + "',listeners:{click: function(b,e){toggleOLViewPreview();}}},";
        }

        var oltbar = [
            "tbar:[" + olpre + "{icon: 'img//new.gif',cls:'x-btn-icon',tooltip: '" + txtNewHere + "',listeners:{click: function(b,e){addOLItemHere(null,'" + folder.EntryID + "');}}},",
        ...
        if (Browser.Version() >= 7 && conStatus == "Outlook ActiveX") {
            oltbar = "tbar:[" + olpre + "{text: 'Browser does not support this',handler:function(b,e){alert('IE 7 now blocks handling of Outlook view control contents. You can manage contents only by the built in keyboard shortcuts or context menus.');}}],";
        }

        ret = [
            "new Ext.Panel({frame:false,border:false,id:'olview:" + id + "',",
...
//set widget title
        setTimeout(function() {
            var pnl = Ext.getCmp("w" + id);
            try {
                ttl = folder.FolderPath + " (" + folder.Items.Count + ")";
            } catch (e) {
                ttl = "Undefined Folder";
            }
            if (ttl.length > 25) {
                ttl = ttl.substr(0, 10) + "..." + ttl.slice((ttl.length - 21), ttl.length);
            }
            try {
                pnl.setTitle(ttl);
            } catch (e) {
            ...
            try {
                var ov = document.getElementById("olv:" + id);
                ov.View = ws[1];
            } catch (e) {
            }
...
//tag folder selection dialog
    ...
    if (noViews && t.DefaultItemType != 0 && !allTypes) {
        alert(txtMailFolderPrompt);
        return;
    }
    if (t != null) {
...
//Settings:0=Widget Title, 1=URL
    ...
    if (wPage.arguments.length > 1) { //customize
...
function ticklerPopup() {
    ...
    var dateMenu = new Ext.menu.DateMenu({
        handler: function(dp, date) {
            var a = new Date(date);
            var b = new Date(date);
            b.setDate(b.getDate() + parseInt(jello.ticklerPopupDuration));
...
//Settings:0=Widget Title, 1=Show inactive projects
    ...
    if (wProject.arguments.length > 1) { //customize
...
//set widget's title
    setTimeout(function() {
        try {
            Ext.getCmp("w" + id).setTitle(txtWidProjects);
        } catch (e) {
        }
    }, 3);
    return ret;
...
function wProjectsContent(showInactive, showPath, id) {
    ...
    var showInac = false;
    if (trimLow(showInactive).substr(0, 4) == "only") {
        showInac = true;
        filteron = txtInactive;
    }
    var showBoth = false;
    if (trimLow(showInactive).substr(0, 4) == "both") {
        showBoth = true;
        filteron = txtAll;
    }
    var showPth = false;
    if (trimLow(showPath) == "yes") {
        showPth = true;
    }
    ...
    var renderTag = function(r) {
        ...
        var aall = 0;
        aall = showContext(tname, 0, null, 4);
...
//for (var x=tgCount;x<=3;x++){ret+="<td colspan=2>&nbsp;</td>";}ret+="</tr>";
    ...
    setTimeout(function() {
        try {
            Ext.getCmp("w" + id).setTitle(txtWidProjects + "-" + filteron + " (" + tgCount + ")");
        } catch (e) {
        }
    }, 6);
    return ret;
    ...
    if (event.keyCode == 13) {
        ...
        if (tg != null) {
            ...
            var tgfind = function(r) {
                ...
                vl = Ext.escapeRe(vl);
                if (trimLow(vl).search(tg) > -1) {
                    foundTags.push(r.get("id"));
                }
            };
            ...
            if (foundTags.length == 1) {
                WQuickListAdd(val, eType, foundTags[0]);
            } else {
                //multiple tags
...
//quick entry tag selector window
    ...
    win.setActive(true);
    setTimeout(function() {
        var ste = document.getElementById("SELECTTAG");
        ste.focus();
    }, 16);
...
//add quick item to outlook
    ...
    if (etype == 0) { //action
        ...
        if (tg != "[" + txtInbox + "]") {
            newA.itemProperties.item(catProperty).Value = tg;
        }
        newA.Save();
    ...
    if (etype == 1) { //tag
        ...
        if (!createTag(val, false, sval, false, true, false)) {
            return;
        }
    ...
    if (etype == 3) { //project
        ...
        if (!createTag(val, false, sval, false, true, true)) {
            return;
        }
        var cval = val.replace(new RegExp(" ", "g"), "~");
    ...
    del.focus;
    try {
        Ext.getCmp("thetagselectform").destroy();
    } catch (e) {
    }
}
...
//Settings:0=Outlook inbox folder ID
    ...
    var ws = getWidgetSettings(id);
    var folder = setAndCheckArcFolder(ws[0]);
    if (wJDInbox.arguments.length == 2) { //customize
        var ffpath = "<No Folder>", ffid = 0;
        try {
            ffpath = folder.FolderPath;
            ffid = folder.EntryID;
        } catch (e) {
        }
...
//set widget's contents
        ...
        if (typeof(wJInboxActive[id]) == "undefined") {
            wJInboxActive[id] = 1;
        }
//create tags list
        tagStore.filter("istag", true);
...
*/
        ...
        var ret = [
            "new Ext.Panel({id:'wci00',frame:false,margins:'5 5 5 5',",
    ...
    setTimeout(function() {
        try {
            Ext.getCmp("w" + id).setTitle(txtInbox + " " + folder.FolderPath);
        } catch (e) {
        }
    }, 30);
...
//mail items
    ...
    if (counter == 0) {
        ret = "<p align=center><br><b>Inbox Empty!</b><br>wow...</p>";
        return ret;
    }

    if (num == 0) {
        num = counter;
        wJInboxActive[iid] = num;
    }
    if (num > counter) {
        num = 1;
        wJInboxActive[iid] = num;
...
//wJInboxActive[iid]++;
    ...
    if (it.Importance == 2) {
        mailic = "widgetimpmail title='High Importance'";
    }
    ...
    var msgCls = "";
    if (unr) {
        msgCls = " style='font-weight:bold'";
    }
    var dp = getJDueProperty(it);
    var itDue = "";
    if (notEmpty(dp)) {
        itDue = "<span onclick=WJInboxFunction(13,'" + iid + "'); title='" + txtDueDate + ". Click to remove' class=tagnext style='cursor:hand;'>" + DisplayAppDate(new Date(dp)) + "</span>";
    }
    if (atts > 0) {
        attLink = "&nbsp;<a style:'cursor:hand;' title='" + atts + " " + txtAttachments + "'><span style='font: x-small wingdings;font-size:17px;color:gray'>2</span>" + atts + "</a>";
    }
    if (flg == 2) {
        attLink = "&nbsp;<span style='font: x-small wingdings;font-size:17px;color:red'>P</span>" + attLink;
    }
    ...
    if (notEmpty(tList[0])) {
        ret += "&nbsp;<a class=jellolinktop style='font-size:9px;' onclick=WJInboxFunction(10," + iid + "); title='Click to copy tags from this message'>[CP]</a>";
    }
    if (notEmpty(wJInboxClipbrd[iid])) {
        ret += "&nbsp;<a class=jellolinktop style='font-size:9px;' onclick=WJInboxFunction(11," + iid + "); title='Click to paste [" + wJInboxClipbrd[iid] + "] to this message'>[PS]</a>";
    }
...
//		}
    ...
    if (fun == 6) { //tag item
        ...
        if (notEmpty(tg)) {
            if (addTagTo(tg, it, null, true) == false) {
                return;
            }
        }
        doWidgetRefresh(Ext.getCmp("w" + id), id);
    ...
    if (fun == 12) { //set due date
        var it = NSpace.GetItemFromID(eid);
        setDueDate(it, dt);
        doWidgetRefresh(Ext.getCmp("w" + id), id);
    }

    if (fun == 13) { //unset due date
        var it = NSpace.GetItemFromID(eid);
...
//Settings:0=Outlook inbox folder ID
    ...
    if (wFolSearch.arguments.length == 5) { //set only title
        setTimeout(function() {
            try {
                Ext.getCmp("w" + id).setTitle(txtFind + " " + folder);
                return;
            } catch (e) {
            }
        }, 30);
    ...
    if (wFolSearch.arguments.length == 2) { //customize
...
//execute quick search for OL folder widget
    if (event.keyCode == 13) {
        ...
        if (olf.DefaultItemType == 2) { /*Contact*/
            fld = "urn:schemas:contacts:cn";
            var fname = "FullName";
            iimg = "user.gif";
        }
        if (olf.DefaultItemType == 1) { /*appointment*/
            iimg = "appoint.gif";
        }
        if (olf.DefaultItemType == 4) { /*journal*/
            iimg = "appoint.gif";
        }
        if (olf.DefaultItemType == 0) { /*mail*/
            iimg = "inboxmail.gif";
        }
        if (olf.DefaultItemType == 5) { /*note*/
            iimg = "note.gif";
        }
        if (olf.DefaultItemType == 6) { /*post*/
            iimg = "post.gif";
        }
        if (olf.DefaultItemType == 3) { /*task*/
            iimg = "task.gif";
        }
